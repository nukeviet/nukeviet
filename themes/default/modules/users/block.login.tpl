...
<form action="{USER_LOGIN}" method="post" class="login clearfix">
	<fieldset>
		...
		<!-- BEGIN: captcha -->
		<p>
			<label for="block_vimg"> {LANG.securitycode} </label>
			...
			<label for="block_seccode_iavim"> {LANG.securitycode} </label>
		...
		<div style="padding-top:10px;">
			<label> <img style="margin-right:3px;vertical-align:middle;" alt="{LANG.openid_login}" src="{OPENID_IMG_SRC}" width="{OPENID_IMG_WIDTH}" height="{OPENID_IMG_HEIGHT}" /> {LANG.openid_login} </label>
			<!-- BEGIN: server -->
