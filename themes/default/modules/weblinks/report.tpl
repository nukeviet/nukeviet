	...
	<body onload="self.focus()">
		...
		<form method="post" action="{ROW.action}">
			<table class="tab1">
				...
				<tr>
					<td><input type="checkbox"  value="1" id="others"/><strong>{LANG.report_note}</strong></td>
				</tr>
				<tbody id="other_show" style="display:none">
					<tr>
						<td><textarea rows="3" style="width:98%; font-size:12px; font-family:Arial" name="report_note" id="report_3">{ROW.report_note}</textarea></td>
					</tr>
				</tbody>
				<tr>
					<td><input type="hidden" name="report_id" value="{ROW.id}" /><input type="hidden" name="link" value="{ROW.link}" /><input type="submit" name="submit" value="{LANG.report_confirm}" style="text-align: center;"/></td>
				</tr>
			</table>
		</form>
		<!-- BEGIN: success -->
