...
function nv4_block_r1( $block_config )
{
	...
	$xtpl = new XTemplate( 'global.r1.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks' );
	$xtpl->assign( 'NV_BASE_SITEURL', NV_BASE_SITEURL );
	$xtpl->assign( 'TEMPLATE', $global_config['module_theme'] );

$sql = 'SELECT  `title`, `dis`, `url` FROM `nv_sidebar1`';
	$q = $db->query($sql);
	$arr = $q->fetchAll();
...
{
	$content = nv4_block_r1( $block_config );
}
