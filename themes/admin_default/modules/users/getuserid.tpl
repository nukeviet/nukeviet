...
<table class="tab1">
	...
	<tbody {CLASS}>
		<tr>
			<td style="width:30px;"><strong>{ROW.userid}</strong></td>
			<td style="width:100px;"> {ROW.username} </td>
			<td> {ROW.email} </td>
			<td style="width:100px;white-space:nowrap;"> {ROW.regdate} </td>
			<td style="width:50px;text-align:center"><a title="" onclick="nv_close_pop('{ROW.userid}');" href="javascript:void(0);">{LANG.select}</a></td>
		</tr>
	</tbody>
	<!-- END: row -->
	<!-- BEGIN: generate_page -->
