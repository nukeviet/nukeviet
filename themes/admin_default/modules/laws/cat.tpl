...
<div class="myh3"><span><a class="mycat" href="0">{LANG.cat}</a></span></div>
...
<table class="table table-striped table-bordered table-hover" summary="{PARENTID}">
    ...
    <!-- BEGIN: loop -->
        <tr>
            ...
            <td>
            <a href="{MODULE_URL}=cat&edit&id={LOOP.id}">{GLANG.edit}</a> | <a class="del" href="{LOOP.id}">{GLANG.delete}</a>
            </td>
        </tr>
    <!-- END: loop -->
    <tbody>
...
$("select.newWeight").change(function() {
  ...
  $.ajax({type:"POST", url:"{MODULE_URL}=cat", data:"cWeight=" + b + "&id=" + a, success:function(a) {
