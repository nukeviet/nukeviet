...
<table class="table table-striped table-bordered table-hover">
	<thead>
		<tr>
			...
			<td>{LANG.publtime}</td>
			<td>{LANG.exptime}</td>
			<td class="text-center" style="width:100px">{LANG.status}</td>
			<td class="text-center" style="width:100px">{LANG.feature}</td>
		</tr>
	</thead>
	<tbody>
	<!-- BEGIN: loop -->
		<tr>
			...
			<td class="text-center">
				<span class="edit_icon"><a href="{DATA.url_edit}">{GLANG.edit}</a></span>
				&nbsp;&nbsp;
				<span class="delete_icon"><a href="javascript:void(0);" onclick="nv_delete_law({DATA.id});">{GLANG.delete}</a></span>
			</td>
		</tr>
	<!-- END: loop -->
	<tbody>
...
<!-- END: list -->
<!-- BEGIN: main -->
<div style="text-align: left; margin-bottom:10px;"><input name="submit" onclick="window.location='{ADD_LINK}';" type="button" value="{LANG.add}" /></div>
<div id="lawlist">
	...
	<form class="form-inline" id="addRow" action="{DATA.action_url}" method="post">
		...
		<table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td>{LANG.code}</td>
                    <td><input title="{LANG.code}" class="form-control txt" type="text" name="code" value="{DATA.code}" maxlength="255" /></td>
                </tr>
                <tr>
                    <td>{LANG.catSel}</td>
                    <td>
                ...
                <tr>
                    ...
                    <!--<input title="{LANG.signer}" class="form-control txt" type="text" name="signer" value="{DATA.signer}" maxlength="255" />-->
                    </td>
                </tr>
                <tr>
                    <td>{LANG.who_view}</td>
                ...
                <tr>
                    <td style="vertical-align:top">
						{LANG.fileupload} <strong>[<a onclick="nv_add_files('{NV_BASE_ADMINURL}','{UPLOADS_DIR_USER}','{GLANG.delete}','Browse server');" href="javascript:void(0);" title="{LANG.add}">{LANG.add}]</a></strong>
					</td>
                    <td>
						<div id="filearea">
							<!-- BEGIN: files -->
							<div id="fileitem_{FILEUPL.id}">
								<input title="{LANG.fileupload}" class="form-control txt" type="text" name="files[]" id="fileupload_{FILEUPL.id}" value="{FILEUPL.value}" />
								<input onclick="nv_open_browse( '{NV_BASE_ADMINURL}index.php?{NV_NAME_VARIABLE}=upload&popup=1&area=fileupload_{FILEUPL.id}&path={UPLOADS_DIR_USER}&type=file', 'NVImg', '850', '500', 'resizable=no,scrollbars=no,toolbar=no,location=no,status=no' );return false;" type="button" value="Browse server" class="selectfile" />
								<input onclick="nv_delete_datacontent('fileitem_{FILEUPL.id}');return false;" type="button" value="{GLANG.delete}" class="selectfile" />
							</div>
							<!-- END: files -->
						</div>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input class="form-control" name="publtime" id="publtime" value="{DATA.publtime}" style="width: 90px;" maxlength="10" type="text" />
                        ({LANG.prm})
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input class="form-control" name="startvalid" id="startvalid" value="{DATA.startvalid}" style="width: 90px;" maxlength="10" type="text" />
                        ({LANG.prm})
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
						<select class="form-control" id="chooseexptime" name="chooseexptime">
						...
						<div id="exptimearea" style="display:{DATA.display}">
							<input class="form-control" name="exptime" id="exptime" value="{DATA.exptime}" style="width: 90px;" maxlength="10" type="text" />
                ...
                <tr>
                    <td>
                        {LANG.replacement} ({LANG.ID})
                    </td>
                    <td>
                        <input class="form-control" title="{LANG.replacement}" type="text" name="replacement" id="replacement" style="width: 190px;" maxlength="255" value="{DATA.replacement}" />
                    </td>
                </tr>
                <tr>
                    <td>
                        {LANG.relatement} ({LANG.ID})
                    </td>
                    <td>
                        <input class="form-control" title="{LANG.relatement}" type="text" name="relatement" id="relatement" style="width: 190px;" maxlength="255" value="{DATA.relatement}" />
                    </td>
                </tr>
                <tr>
                    <td>{LANG.keywords}</td>
                    <td><input title="{LANG.keywords}" class="form-control txt" type="text" name="keywords" value="{DATA.keywords}" maxlength="255" /> ({LANG.keywordsNote})</td>
                </tr>
                <tr>
                    <td style="vertical-align:top">{LANG.note}</td>
                    <td><textarea style="width:280px;height:50px" name="note" id="note">{DATA.note}</textarea>
                    </td>
                </tr>
                <tr>
                    <td style="vertical-align:top">{LANG.introtext}</td>
                    <td><textarea style="width:280px;height:50px" name="introtext" id="introtext">{DATA.introtext}</textarea>
                    </td>
                </tr>
            </tbody>
        </table>
        <div>{LANG.bodytext}</div>
        <div>{CONTENT}</div>
