...
<form name="myform" id="myform" method="post" action="{FORM_ACTION}">
	<table class="tab1">
		...
		<tfoot>
			<tr>
				<td><input name="check_all[]" type="checkbox" value="yes" onclick="nv_checkAll(this.form, 'sends[]', 'check_all[]',this.checked);" /></td>
				<td colspan="5"><i class="icon-trash icon-large">&nbsp;</i> <a href="javascript:void(0);" onclick="nv_del_submit(document.myform, 'sends[]');">{LANG.bt_del_row_title}</a> &nbsp; <i class="icon-trash icon-large">&nbsp;</i> <a href="javascript:void(0)" onclick="nv_delall_submit();">{LANG.delall}</a></td>
			</tr>
		</tfoot>
		<tbody>
			<!-- BEGIN: row -->
