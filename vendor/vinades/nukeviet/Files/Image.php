...
class Image
{
    ...
    private static function set_memory_limit($fileinfo)
    {
        ...
        $memoryHave = self::function_exists('memory_get_usage') ? @memory_get_usage() : 0;
    ...
    private function set_tempnam($filename)
    {
        ...
        if (defined('CURLPROTO_HTTP') and defined('CURLPROTO_HTTPS')) {
    ...
    private function is_safe_url_target($url)
    {
        ...
        $pin = null;
        if (!\NukeViet\Core\Ips::is_safe_host($host, $pin, defined('NV_DEVELOPER_MODE'))) {
            return false;
        }
    ...
    public function resizeXY($maxX = 0, $maxY = 0)
    {
        if (empty($this->error)) {
            ...
            if (($maxX != 0 or $maxY != 0) and ($maxX != $this->create_Image_info['width'] or $maxY != $this->create_Image_info['height'])) {
                ...
                $workingImage = self::function_exists('ImageCreateTrueColor') ? imagecreatetruecolor($newwidth, $newheight) : imagecreate($newwidth, $newheight);
    ...
    public function addlogo($logo, $align = 'right', $valign = 'bottom', $config_logo = [])
    {
        if (empty($this->error)) {
            ...
            if (
                $logo_info != [] and $logo_info['width'] != 0 and $logo_info['height'] != 0
