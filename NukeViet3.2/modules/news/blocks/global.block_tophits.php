...
if ( ! nv_function_exists( 'nv_news_block_tophits' ) )
{
    ...
    function nv_news_block_tophits ( $block_config, $mod_data )
    {
        ...
        $sql = "SELECT id, listcatid, publtime, exptime, title, alias, homeimgthumb, homeimgfile FROM `" . NV_PREFIXLANG . "_" . $mod_data . "_rows` WHERE `status`= 1 AND `publtime` BETWEEN  " . $publtime . " AND " . NV_CURRENTTIME . " AND (`exptime`=0 OR `exptime`>" . NV_CURRENTTIME . ") ORDER BY `hitstotal` DESC LIMIT 0 , " . $block_config['numrow'];
...
if ( defined( 'NV_SYSTEM' ) )
{
    ...
    if ( isset( $site_mods[$module] ) )
    {
        ...
        else
        {
            ...
            $sql = "SELECT catid, parentid, title, alias, viewcat, subcatid, numlinks, del_cache_time, description, inhome, keywords, who_view, groups_view FROM `" . NV_PREFIXLANG . "_" . $mod_data . "_cat` ORDER BY `order` ASC";
