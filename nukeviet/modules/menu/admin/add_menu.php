...
{
    $list_module[] = array("key" => $key, "title" => $title['module_data'], "selected" => ($key == $post['module_name']) ? " selected=\"selected\"" : "", );
}

$list_target = array();
foreach ($type_target as $key => $target)
{
    $list_target[] = array("key" => $key, "title" => $target, "selected" => ($key == $post['target']) ? " selected=\"selected\"" : "", );
}
...
if (!empty($groups_list))
{
    foreach ($groups_list as $key => $title)
    {
        ...
        {
            $array['groups_view'][] = array('key' => $key, //
                'title' => $title, //
    ...
    if ($post['op'] != "")
    {
        ...
        if (($result = $db->sql_query($sql)) == FALSE)
        {
            ...
            if ($result = $db->sql_query($sql) == TRUE)
            {
                ...
                While ($row = $db->sql_fetchrow($result))
                {
                    $arr_cat[$row['id']] = array('module' => $post['module_name'], //
                        'id' => $row['id'], //
                        'title' => $row['title'], //
                        'alias' => $row['alias'], //
        ...
        else
        {
            ...
            While ($row = $db->sql_fetchrow($result))
            {
                $arr_cat[$row['catid']] = array('module' => $post['module_name'], //
                    'key' => $row['catid'], //
                    'title' => $row['title'], //
                    'alias' => $row['alias'], //
                    "selected" => ($row['alias'] == $post['op']) ? " selected=\"selected\"" : ""  //
