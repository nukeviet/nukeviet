...
list( $new ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM `" .
    NV_BANNERS_ROWS_GLOBALTABLE . "` WHERE `act`=4" ) );
if ( $new > 0 )
{
...
list( $deact ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM `" .
    NV_BANNERS_ROWS_GLOBALTABLE . "` WHERE `act`=3" ) );
if ( $deact > 0 )
{
...
list( $exp ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM `" .
    NV_BANNERS_ROWS_GLOBALTABLE . "` WHERE `act`=2" ) );
if ( $exp > 0 )
{
...
{
    Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" .
        $module_name . "&" . NV_OP_VARIABLE . "=banners_list" );
    die();
}
