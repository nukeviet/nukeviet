...
if ($op == "main")
{
    ...
    if (!empty($list_cats))
    {
        ...
        foreach ($list_cats as $c)
        {
            ...
            if ($module_info['rss'])
            {
                $rss[] = array('title' => $module_info['custom_title'] . ' - ' . $c['title'], //
        ...
        if ($catid > 0)
        {
            ...
            while ($parentid > 0)
            {
                ...
                $array_mod_title[] = array('catid' => $parentid, 'title' => $c['title'], 'link' => NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $c['alias']);
                $parentid = $c['parentid'];
            }
