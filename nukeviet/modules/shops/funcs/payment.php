...
if ( $order_id > 0 and $checkss == md5( $order_id . $global_config['sitekey'] . session_id() ) )
{
    ...
    $re = $db->sql_query( "SELECT * FROM `" . $db_config['prefix'] . "_" . $module_data . "_orders` WHERE order_id=" . $order_id );
    $data = $db->sql_fetchrow( $re );
    
    if ( empty( $data ) )
    {
        Header( "Location: " . NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=cart" );
        die();
    }
    ...
    if ( ! empty( $arrayid ) )
    {
        ...
        {
            $data_pro[] = array( 
                "id" => $id, "publtime" => $publtime, "title" => $title, "alias" => $alias, "product_note" => $note, "hometext" => $hometext, "product_price" => $temppro[$id]['price'], "product_unit" => $unit, "money_unit" => $money_unit, "link_pro" => $link . $global_array_cat[$listcatid]['alias'] . "/" . $alias . "-" . $id, "product_number" => $temppro[$id]['num'] 
            );
        }
    }
    ...
    elseif ( $data['transaction_status'] == 0 )
    {
        ...
        while ( $row = $db->sql_fetchrow( $result ) )
        {
            if ( file_exists( NV_ROOTDIR . "/modules/" . $module_file . "/payment/" . $row['payment'] . ".checkout_url.php" ) )
            {
                ...
                $url_checkout[] = array( 
                    "name" => $row['paymentname'], "url" => $url, "images_button" => $images_button 
                );
            }
        }
        // get content intro 
...
elseif ( $order_id > 0 and $nv_Request->isset_request( 'payment', 'get' ) and $nv_Request->isset_request( 'checksum', 'get' ) )
{
    ...
    $query = $db->sql_query( "SELECT * FROM `" . $db_config['prefix'] . "_" . $module_data . "_orders` WHERE order_id=" . $order_id );
