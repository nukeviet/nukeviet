...
$array_cat_search[0] = array( 
    'catid' => 0, 'title' => $lang_module['search_all'], 'select' => ( 0 == $catid ) ? "selected" : "", 'xtitle' => '' 
);
foreach ( $global_array_cat as $arr_cat_i )
{
    ...
    $array_cat_search[$arr_cat_i['catid']] = array( 
        'catid' => $arr_cat_i['catid'], 'title' => $arr_cat_i['title'], 'select' => ( $arr_cat_i['catid'] == $catid ) ? "selected" : "", 'xtitle' => $xtitle 
    );
}
...
if ( strlen( $key ) >= NV_MIN_SEARCH_LENGTH )
{
    ...
    $sql = " SELECT SQL_CALC_FOUND_ROWS id," . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias,listcatid," . NV_LANG_DATA . "_hometext,publtime,homeimgfile, homeimgthumb,source_id
	FROM `" . $table_search . "` 
    ...
    while ( list( $id, $title, $alias, $listcatid, $hometext, $publtime, $homeimgfile, $homeimgthumb, $sourceid ) = $db->sql_fetchrow( $result ) )
    {
        ...
        $array_content[] = array( 
            "id" => $id, "title" => $title, "alias" => $alias, "listcatid" => $listcatid, "hometext" => $hometext, "publtime" => $publtime, "homeimgthumb" => $thumb[0], "sourceid" => $sourceid 
        );
    }
