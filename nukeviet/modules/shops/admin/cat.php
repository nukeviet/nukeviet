...
$savecat = 0;
$data = array();
list( $data['catid'], $data['parentid'], $data['title'], $data['alias'], $data['description'], $data['keywords'], $data['who_view'], $groups_view ) = array( 
    0, 0, "", "", "", "", 0, "" 
);
...
if ( ! empty( $savecat ) )
{
    ...
    if ( $data['catid'] == 0 and $data['title'] != "" and $error == "" )
    {
        ...
        $subcatid = "";
        $query = "INSERT INTO " . $table_name . " (`catid`, `parentid`, `image`, `thumbnail`, `weight`, `order`, `lev`, `viewcat`, `numsubcat`, `subcatid`, `inhome`, `numlinks`, `admins`, `add_time`, `edit_time`, `del_cache_time`, `who_view`, `groups_view` " . $listfield . " ) 
...
$contents .= nv_show_cat_list( $data['parentid'] );
$contents .= "</div>\n";
...
// * call theme tpl of product catalogs */
