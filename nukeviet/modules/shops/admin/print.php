...
$re = $db->sql_query("SELECT *  FROM `" . $table_name . "` WHERE `order_id`=" . $id);
...
foreach ($listid as $id)
{
    ...
    $xtpl->assign('product_price', nv_number_format($product_price));
    $xtpl->assign('product_unit', $money_unit);
    $xtpl->assign('tt', $i + 1);
    $xtpl->parse('main.loop');
    ++$i;
}
$xtpl->assign('order_total', nv_number_format($data['order_total']));
