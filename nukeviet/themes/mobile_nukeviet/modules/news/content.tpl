...
<div class="hr"></div><!-- END: mainrefresh -->
<!-- BEGIN: main -->
<form action="{CONTENT_URL}" method="post">
	...
	<div style="max-height:130px;overflow:auto">
		<!-- BEGIN: catid -->
		<input name="catids[]" value="{DATACATID.value}" type="checkbox"{DATACATID.checked}/>{DATACATID.title}<br /><!-- END: catid -->
	</div>
	{LANG.content_topic}
	<select name="topicid">
		<!-- BEGIN: topic -->
		<option value="{DATATOPIC.value}"{DATATOPIC.selected}>{DATATOPIC.title}</option><!-- END: topic -->
	</select>
	...
	<select name="imgposition">
		<!-- BEGIN: imgposition -->
		<option value="{DATAIMGOP.value}"{DATAIMGOP.selected}>{DATAIMGOP.title}</option><!-- END: imgposition -->
	</select>
...
<div class="ac">
	...
	<!-- BEGIN: save_temp --><input type="submit" value="{LANG.save_temp}" name="status0"> <!-- END: save_temp -->
	<!-- BEGIN: postcontent --> <input type="submit" value="{LANG.save_content}" name="status1"><!-- END: postcontent -->
</div>
</form>
<!-- END: main -->