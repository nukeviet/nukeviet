...
<form action="{BASE_URL_SITE}" method="get">
	...
	<select name="catid">
		<!-- BEGIN: search_cat -->
		<option value="{SEARCH_CAT.catid}" {SEARCH_CAT.select}>{SEARCH_CAT.title}</option><!-- END: search_cat -->
	</select>
	{LANG.finter_title}<span class="smll"> ({LANG.search_date_rule})</span><br />
	<input name="from_date" value="{FROM_DATE}" maxlength="10" type="text"/>
	{LANG.to_date}<span class="smll"> ({LANG.search_date_rule})</span><br />
...
<!-- BEGIN: noneresult -->
{LANG.search_none} : "<b>{KEY}</b>" {LANG.search_in_module} <b>{INMOD}</b><!-- END: noneresult -->
<!-- BEGIN: result -->
<!-- BEGIN: result_img -->
<a href="{LINK}"><img src="{IMG_SRC}" width="{IMG_WIDTH}" class="fl"/></a><!-- END: result_img -->
<h3><a href="{LINK}">{TITLEROW}</a></h3>
{CONTENT}<br />
<span class="smll">{AUTHOR} | {LANG.source_title} : {SOURCE}</span>
<div class="hr clear"></div>
<!-- END: result --><!-- BEGIN: pages_result -->
<br />{VIEW_PAGES}<!-- END: pages_result -->
<p><i>{LANG.search_sum_title} {NUMRECORD} {LANG.result_title}<br />{LANG.info_adv}</i></p>
