...
if ($module_name == "settings")
{
    ...
    function nv_admin_add_theme($contents)
    {
        ...
        $return .= "<col valign=\"top\" width=\"300px\" />\n";
        $return .= "<tr>\n";
        $return .= "<td>" . $contents['start_time'][0] . ":</td>\n";
        $return .= "<td></td>\n";
        $return .= "<td><select name=\"hour\">\n";
        for ($i = 0; $i < 24; ++$i)
        {
            $return .= "<option value=\"" . $i . "\"" . ($i == $contents['hour'][1] ? " selected=\"selected\"" : "") . ">" . $i . "</option>\n";
        }
        $return .= "</select> " . $contents['hour'][0] . " <select name=\"min\">\n";
        for ($i = 0; $i < 60; ++$i)
        {
            $return .= "<option value=\"" . $i . "\"" . ($i == $contents['min'][1] ? " selected=\"selected\"" : "") . ">" . $i . "</option>\n";
        }
        $return .= "</select> " . $contents['min'][0] . "\n";
        $return .= ",  " . $contents['start_time'][1] . " <input name=\"start_date\" id=\"start_date\" value=\"" . $contents['start_time'][2] . "\" style=\"width: 90px;\" maxlength=\"10\" readonly=\"readonly\" type=\"text\" />\n";
        $return .= "<td></td>\n";
        $return .= "</tr>\n";
        $return .= "</table>\n";
