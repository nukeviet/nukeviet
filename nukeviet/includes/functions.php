...
function nv_is_url( $url )
{
	...
	if( ! preg_match( "/^[0-9a-z]([\-\.]?[0-9a-z])*\.(ac|ad|ae|aero|af|ag|ai|al|am|an|ao|aq|ar|arpa|as|asia|at|au|aw|ax|az|ba|bb|bd|be|bf|bg|bh|bi|biz|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cat|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|com|coop|cr|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|edu|ee|eg|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gg|gh|gi|gl|gm|gn|gov|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|im|in|info|int|io|iq|ir|is|it|je|jm|jo|jobs|jp|ke|kg|kh|ki|km|kn|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|mg|mh|mil|mk|ml|mm|mn|mo|mobi|mp|mq|mr|ms|mt|mu|museum|mv|mw|mx|my|mz|na|name|nc|ne|net|nf|ng|ni|nl|no|np|nr|nu|nz|om|org|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|pro|ps|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tel|tf|tg|th|tj|tk|tl|tm|tn|to|tp|tr|travel|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)$/", $parts['host'] ) )
	{
		global $ips;
		
		if( $parts['host'] != 'localhost' and ! $ips->nv_validip( $parts['host'] ) )
		{
			return false;
		}
	}
...
function nv_change_buffer( $buffer )
{
	...
	if( ! $sys_info['supports_rewrite'] ) $buffer = preg_replace( "/\<(script|link)(.*?)(src|href)=['\"]((?!http(s?)|ftp\:\/\/).*?\.(js|css))['\"](.*?)\>/", "<\\1\\2\\3=\"" . NV_BASE_SITEURL . "CJzip.php?file=\\4&amp;r=1\"\\7>", $buffer );
	...
	//Bat buoc phai co alt trong IMG
...
function nv_update_tags( $module, $sid, $keywords, $link, $title, $text, $image, $publtime = 0 )
{
	...
	$tableid_array = array();
	$arr_tableid_con = array();
	$array_tid = array( 0 );

	$result = $db->sql_query( "SHOW TABLE STATUS LIKE '" . nv_preg_quote( NV_PREFIXLANG ) . "\_tags\_con\_%'" );

	while( $itemdb = $db->sql_fetch_assoc( $result ) )
	{
		$tableid_array[] = str_replace( NV_PREFIXLANG . '_tags_con_', '', $itemdb['Name'] );
	}
	...
	if( ! empty( $keywords ) )
	{
		...
		{
			$key = trim( html_entity_decode( $key, ENT_QUOTES, 'UTF-8' ) );
		
			list( $tid ) = $db->sql_fetchrow( $db->sql_query( "SELECT `tid` FROM `" . NV_PREFIXLANG . "_tags` WHERE `keys`=" . $db->dbescape( $key ) ) );
		
			if( empty( $tid ) )
			{
				$tid = $db->sql_query_insert_id( "INSERT INTO `" . NV_PREFIXLANG . "_tags` (`tid`, `keys`, `total`) VALUES (NULL, " . $db->dbescape( $key ) . ", '0')" );
			...
			if( ! in_array( $tableid, $tableid_array ) )
			{
				$tableid_array[] = $tableid;
				
				$db->sql_query( "CREATE TABLE IF NOT EXISTS `" . NV_PREFIXLANG . "_tags_con_" . $tableid . "` (
			...
			{
				$db->sql_query( "UPDATE `" . NV_PREFIXLANG . "_tags` SET total=total+1 WHERE `tid`=" . $tid );
			}
		}
	}
	
	foreach( $tableid_array as $tableid )
	{
		$result = $db->sql_query( "SELECT tid FROM `" . NV_PREFIXLANG . "_tags_kid_" . $tableid . "` WHERE `module` = '" . $module . "' AND `sid` = " . $sid . " AND `tid` NOT IN (" . implode( ',', $array_tid ) . ")" );
	
		while( list( $tid ) = $db->sql_fetchrow( $result, 1 ) )
		{
			$db->sql_query( "UPDATE `" . NV_PREFIXLANG . "_tags` SET total=total-1 WHERE `tid`=" . $tid );
		}
		...
		if( empty( $keywords ) )
		{
			$db->sql_query( "DELETE FROM `" . NV_PREFIXLANG . "_tags_con_" . $tableid . "` WHERE `module` = '" . $module . "' AND `sid` = " . $sid );
		}
	}
}
