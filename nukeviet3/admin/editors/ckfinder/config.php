...
{
    require_once ( str_replace( "\\\\", "/", dirname( __file__ ) ) . "/../level.php" );
}

function CheckAuthentication ( )
{
    return defined( 'NV_IS_ADMIN' ) ? true : false;
}
...
//NUKEVIET
//$baseUrl = '/uploads/';
$baseUrl = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/';
//$baseDir = resolveUrl($baseUrl);
$baseDir = NV_UPLOADS_REAL_DIR . '/';
//END NUKEVIET
...
$config['Thumbnails'] = Array( 
    'url' => $baseUrl . '_thumbs', 'directory' => $baseDir . '_thumbs', 'enabled' => true, 'directAccess' => false, 'maxWidth' => 100, 'maxHeight' => 100, 'bmpSupported' => false, 'quality' => 80 
);
...
*/
$config['Images'] = Array( 
    'maxWidth' => 1600, 'maxHeight' => 1200, 'quality' => 80 
);
...
$config['AccessControl'][] = Array( 
    'role' => '*', 'resourceType' => '*', 'folder' => '/', 'folderView' => true, 'folderCreate' => true, 'folderRename' => true, 'folderDelete' => true, 'fileView' => true, 'fileUpload' => true, 'fileRename' => true, 'fileDelete' => true 
);
...
if ( ! empty( $nv_editor['allow_files_type'] ) )
{
    
    $config['ResourceType'][] = Array( 
        'name' => 'Files', 'url' => $baseUrl, 'directory' => $baseDir, 'maxSize' => 0, 'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,fla,flv,gif,gz,gzip,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,zip', 'deniedExtensions' => '' 
    );
}
if ( in_array( 'images', $nv_editor['allow_files_type'] ) )
{
    $config['ResourceType'][] = Array( 
        'name' => 'Images', 'url' => $baseUrl, 'directory' => $baseDir, 'maxSize' => 0, 'allowedExtensions' => 'bmp,gif,jpeg,jpg,png', 'deniedExtensions' => '' 
    );
}

if ( in_array( 'flash', $nv_editor['allow_files_type'] ) )
{
    $config['ResourceType'][] = Array( 
        'name' => 'Flash', 'url' => $baseUrl, 'directory' => $baseDir, 'maxSize' => 0, 'allowedExtensions' => 'swf,flv', 'deniedExtensions' => '' 
    );
}
