...
$selectedmodule = '';
$whereclause = '';
$selectedmodule = filter_text_input( 'module', 'get', '');
...
{
    $whereclause = 'WHERE theme="' . $selectthemes . '"';
}
...
$result = $db->sql_query( "SELECT bid, groupbl, title, type, file_path, position, all_func, func_id, weight, exp_time, active FROM `" . NV_BLOCKS_TABLE . "` $whereclause GROUP BY `groupbl` ASC" );
while ( $row = $db->sql_fetchrow( $result ) )
{
    ...
    $contents .= "<td>";
    if ( $row['all_func'] == 1 and empty( $selectedmodule ) )
    {
    ...
    else
    {
        ...
        {
            $result_func = $db->sql_query( "SELECT DISTINCT a.func_id,b.in_module, b.func_custom_name FROM `" . NV_BLOCKS_TABLE . "` a INNER JOIN `" . NV_MODFUNCS_TABLE . "` b ON a.func_id=b.func_id WHERE in_module=" . $db->dbescape( $selectedmodule ) . "" );
        }
...
$(function(){
	$("select[name=module]").change(function(){
		var module = $(this).val();
		window.location="index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=blocks&module="+module;
	});
