	...
	if (is_uploaded_file ( $_FILES ['themefile'] ['tmp_name'] ))
	{
		if (move_uploaded_file ( $_FILES ['themefile'] ['tmp_name'], $filename ))
		{
			...
			if ($status ['status'] == 'ok')
			{
				...
				$filelist = '';
				$validfolder = array();
				$filesize = (round ( $_FILES ['themefile'] ['size'] / 1024, 2 ) > 1024) ? ((round ( $_FILES ['themefile'] ['size'] / (pow ( 1024, 2 )), 2 )) > 1024) ? (round ( $_FILES ['themefile'] ['size'] / (pow ( 1024, 3 )), 2 )) . 'GB' : (round ( $_FILES ['themefile'] ['size'] / (pow ( 1024, 2 )), 2 )) . 'MB' : round ( $_FILES ['themefile'] ['size'] / 1024, 2 ) . ' KB';
