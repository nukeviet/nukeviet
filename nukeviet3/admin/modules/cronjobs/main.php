...
if ( ! defined( 'NV_IS_FILE_CRONJOBS' ) )
	die( 'Stop!!!' );
...
{
	$contents[$row['id']]['caption'] = $row['cron_name'];
	$contents[$row['id']]['edit'] = array( ( empty( $row['is_sys'] ) ? 1 : 0 ), $lang_global['edit'], NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=edit&amp;id=" . $row['id'] );
	$contents[$row['id']]['delete'] = array( ( empty( $row['is_sys'] ) ? 1 : 0 ), $lang_global['delete'] );
	$contents[$row['id']]['disable'] = array( ( empty( $row['is_sys'] ) ? 1 : 0 ), ( $row['act'] ? $lang_global['disable'] : $lang_global['activate'] ), NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=act&amp;id=" . $row['id'] );
