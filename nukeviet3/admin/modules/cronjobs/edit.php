...
if ( $nv_Request->get_int( 'save', 'post' ) == '1' )
{
	...
	else
	{
		...
		$sql = "UPDATE `" . NV_CRONJOBS_GLOBALTABLE . "` SET `cron_name`=" . $db->dbescape( $cron_name ) . ", `start_time`=" . $start_time . ", `interval`=" . $interval . ", 
			`run_file`=" . $db->dbescape( $run_file ) . ", `run_func`=" . $db->dbescape( $run_func ) . ", `params`=" . $db->dbescape( $params ) . ", `del`=" . $del . " WHERE `id`=" . $id;
		$db->sql_query( $sql );
		Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name );
		die();
	}
...
$contents['action'] = NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=edit&amp;id=" . $id;
...
$contents['day'] = array( $lang_module['day'], $day );
$contents['month'] = array( $lang_module['month'], $month );
$contents['year'] = array( $lang_module['year'], $year );
