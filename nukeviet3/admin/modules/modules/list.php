...
while ( $row = $db->sql_fetchrow( $result ) )
{
    ...
    $mod['in_menu'] = array( 
        $row['in_menu'], "nv_chang_in_menu('" . $row['title'] . "');" 
    );
    $mod['submenu'] = array( 
        $row['submenu'], "nv_chang_submenu('" . $row['title'] . "');" 
    );
    $mod['weight'] = array( 
        $row['weight'], "nv_chang_weight('" . $row['title'] . "');" 
    );
    $mod['act'] = array( 
    ...
    $mod['recreate'] = array( 
        "nv_recreate_mod('" . $row['title'] . "');", $lang_global['recreate'] 
    );
    $mod['del'] = ( $row['is_sysmod'] == 0 or $row['title'] != $row['module_file'] ) ? array( 
        "nv_mod_del('" . $row['title'] . "');", $lang_global['delete'] 
    ) : array();
...
$contents['caption'] = array( 
    $lang_module['caption_actmod'], $lang_module['caption_deactmod'], $lang_module['caption_badmod'], $lang_module['caption_newmod'] 
);
