...
if ( $numrows )
{
    while ( $row = $db->sql_fetchrow( $result ) )
    {
        ...
        $level_txt = ( $level == 1 ) ? $lang_module['level1'] : ( ( $level == 2 ) ? $lang_module['level2'] : $lang_module['level3'] );
        ...
        $browser = array_combine( array( 
            'key', 'name' 
        ), explode( "|", nv_getBrowser( $last_agent, NV_ROOTDIR . '/includes/ini/br.ini' ) ) );
        
        $os = array_combine( array( 
            'key', 'name' 
        ), explode( "|", nv_getOs( $last_agent, NV_ROOTDIR . '/includes/ini/os.ini' ) ) );
        ...
        {
            $thead['edit'] = array( 
                NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=edit&amp;admin_id=" . $row['admin_id'], $lang_global['edit'] 
            );
        }
        
        if ( ! empty( $thead['chg_is_suspend'] ) )
        {
            $thead['chg_is_suspend'] = array( 
                NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=suspend&amp;admin_id=" . $row['admin_id'], $lang_module['chg_is_suspend2'] 
            );
        }
        if ( ! empty( $thead['del'] ) )
        {
            $thead['del'] = array( 
                NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=del&amp;admin_id=" . $row['admin_id'], $lang_global['delete'] 
            );
        }
        ...
        $admins[$row['admin_id']]['options']['email'] = array( 
            $lang_module['email'], $email 
        );
        $admins[$row['admin_id']]['options']['full_name'] = array( 
            $lang_module['full_name'], $row['full_name'] 
        );
        $admins[$row['admin_id']]['options']['lev'] = array( 
            $lang_module['lev'], $level_txt 
        );
        $admins[$row['admin_id']]['options']['position'] = array( 
            $lang_module['position'], $row['position'] 
        );
        
        if ( defined( 'NV_IS_SPADMIN' ) )
        {
            $admins[$row['admin_id']]['options']['editor'] = array( 
                $lang_module['editor'], ! empty( $row['editor'] ) ? $row['editor'] : $lang_module['not_use'] 
            );
            $admins[$row['admin_id']]['options']['allow_files_type'] = array( 
                $lang_module['allow_files_type'], ! empty( $allow_files_type ) ? implode( ", ", $allow_files_type ) : $lang_global['no'] 
            );
            $admins[$row['admin_id']]['options']['allow_modify_files'] = array( 
                $lang_module['allow_modify_files'], ! empty( $allow_modify_files ) ? $lang_global['yes'] : $lang_global['no'] 
            );
            $admins[$row['admin_id']]['options']['allow_create_subdirectories'] = array( 
                $lang_module['allow_create_subdirectories'], ! empty( $allow_create_subdirectories ) ? $lang_global['yes'] : $lang_global['no'] 
            );
            $admins[$row['admin_id']]['options']['allow_modify_subdirectories'] = array( 
                $lang_module['allow_modify_subdirectories'], ! empty( $allow_modify_subdirectories ) ? $lang_global['yes'] : $lang_global['no'] 
            );
            
            $admins[$row['admin_id']]['options']['regtime'] = array( 
                $lang_module['regtime'], nv_date( "l, d/m/Y H:i", $row['regdate'] ) 
            );
            $admins[$row['admin_id']]['options']['is_suspend'] = array( 
                $lang_module['is_suspend'], $is_suspend 
            );
            $admins[$row['admin_id']]['options']['last_login'] = array( 
                $lang_module['last_login'], $last_login 
            );
            $admins[$row['admin_id']]['options']['last_ip'] = array( 
                $lang_module['last_ip'], $row['last_ip'] 
            );
            $admins[$row['admin_id']]['options']['browser'] = array( 
                $lang_module['browser'], $browser['name'] 
            );
            $admins[$row['admin_id']]['options']['os'] = array( 
                $lang_module['os'], $os['name'] 
            );
        }
    }
}
