...
$query = "SELECT * FROM `" . NV_AUTHORS_GLOBALTABLE . "` WHERE `admin_id`=" . intval( $admin_id );
...
if ( $allow_change )
{
        ...
        else
        {
            if ( $new_suspend )
            {
                if ( $clean_history )
                {
                    $susp_reason = array();
                    $susp_reason[] = array( 
                        'starttime' => NV_CURRENTTIME, 'endtime' => 0, 'start_admin' => $admin_info['admin_id'], 'end_admin' => '', 'info' => $new_reason 
                    );
                }
                else
                {
            ...
            else
            {
                ...
                {
                    $susp_reason[0] = array( 
                        'starttime' => $last_reason['starttime'], 'endtime' => NV_CURRENTTIME, 'start_admin' => $last_reason['start_admin'], 'end_admin' => $admin_info['admin_id'], 'info' => $last_reason['info'] 
                    );
                }
            }
    ...
    $contents['change_suspend']['sendmail'] = array( 
        $lang_module['suspend_sendmail'], $sendmail 
    );
    if ( ! empty( $new_suspend ) )
    {
        $contents['change_suspend']['new_reason'] = array( 
            $lang_module['suspend_reason'], $new_reason, 255 
        );
    }
...
{
    $contents['suspend_info'] = array( 
        sprintf( $lang_module['suspend_info_empty'], $row_user['username'] ), array() 
    );
...
else
{
    ...
    $contents['suspend_info'] = array( 
        sprintf( $lang_module['suspend_info_yes'], $row_user['username'] ), $inf, $lang_module['suspend_start'], $lang_module['suspend_end'], $lang_module['suspend_reason'] 
    );
}
