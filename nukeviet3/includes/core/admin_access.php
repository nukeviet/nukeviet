...
function nv_admin_checkip()
{
    ...
    if ( ! $global_config['block_admin_ip'] ) return true;
    
    $allowed_ip_file = NV_ROOTDIR . "/" . NV_DATADIR . "/admin_ip.config";
    $allowed_admin_ip = ( file_exists( $allowed_ip_file ) and filesize( $allowed_ip_file ) != 0 ) ? unserialize( file_get_contents( $allowed_ip_file ) ) : array();
    if ( $allowed_admin_ip == array() ) return true;

    $allowed_admin_ip = array_map( "nv_preg_quote", $allowed_admin_ip );
    $allowed_admin_ip = implode( "|", $allowed_admin_ip );
    if ( ! empty( $allowed_admin_ip ) and preg_match( "/^" . $allowed_admin_ip . "/", $client_info['ip'] ) ) return true;
    return false;
}
...
function nv_admin_checkfirewall()
{
    ...
    if ( ! $global_config['admfirewall'] ) return true;

    $adv_admin_file = NV_ROOTDIR . "/" . NV_DATADIR . "/admin_adv.config";
    $adv_admins = ( file_exists( $adv_admin_file ) and filesize( $adv_admin_file ) != 0 ) ? unserialize( file_get_contents( $adv_admin_file ) ) : array();
    if ( $adv_admins == array() ) return true;
    $auth = nv_set_authorization();
    if ( empty( $auth['auth_user'] ) || empty( $auth['auth_pw'] ) ) return false;
    foreach ( $adv_admins as $adv_admin )
    {
        if ( $adv_admin['login'] == $auth['auth_user'] and $adv_admin['password'] == md5( $auth['auth_pw'] ) ) return true;
    }
    return false;
}
