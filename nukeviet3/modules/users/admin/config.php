...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	foreach ( $array_config as $config_name => $config_value )
	{
        ...
        `config_value`=" . $db->dbescape( $config_value ) . " 
        WHERE `lang`='sys' AND `module`='global' 
        AND `config_name`=" . $db->dbescape( $config_name );
		$db->sql_query( $query );
	}
...
$array_registertype = array( 0 => $lang_module['active_not_allow'], 1 => $lang_module['active_all'], 2 => $lang_module['active_email'], 3 => $lang_module['active_admin_check'] );

$xtpl = new XTemplate( "config.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_name );
...
foreach ( $array_registertype as $id => $titleregister )
{
	...
	$array = array( "id" => $id, "select" => $select, "value" => $titleregister );
