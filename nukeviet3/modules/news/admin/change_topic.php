...
{
	$query = "SELECT * FROM `" . NV_PREFIXLANG . "_" . $module_data . "_topics` WHERE `topicid`=" . $topicid;
	$result = $db->sql_query( $query );
	$numrows = $db->sql_numrows( $result );
	if ( $numrows != 1 ) die( 'NO_' . $topicid );

	$query = "SELECT `topicid` FROM `" . NV_PREFIXLANG . "_" . $module_data . "_topics` WHERE `topicid`!=" . $topicid . " ORDER BY `weight` ASC";
	$result = $db->sql_query( $query );
	$weight = 0;
