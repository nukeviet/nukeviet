...
while ($row = $db->sql_fetchrow($query))
{
	...
	$topic_array[] = array("id" => $row['id'], "publtime" => $row['publtime'], "title" => $row['title'], "alias" => $row['alias'], "hometext" => $row['hometext'], "alt" => $alt, "src" => $src, "width" => $size[0], "height" => $size[1], "link" => $link);
}
...
while ($row = $db->sql_fetchrow($query))
{
	$catid = explode(",", $row['listcatid']);
	$link = $global_array_cat[$catid[0]]['link']."/".$row['alias']."-".$row['id'];
	$topic_other_array[] = array("title" => $row['title'], "link" => $link, "publtime" => nv_date( "H-m-d", $row['publtime']));
}
unset($query, $row);
