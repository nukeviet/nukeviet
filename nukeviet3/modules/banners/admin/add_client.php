...
if ( $nv_Request->get_int( 'save', 'post' ) == '1' )
{
	...
	else
	{
		...
		$sql = "INSERT INTO `" . NV_BANNERS_CLIENTS_GLOBALTABLE . "` VALUES 
        (NULL, " . $db->dbescape( $login ) . ", " . $db->dbescape( $pass ) . ", " . NV_CURRENTTIME . ", " . $db->dbescape( $full_name ) . ", 
        " . $db->dbescape( $email ) . ", " . $db->dbescape( $website ) . ", " . $db->dbescape( $location ) . ", " . $db->dbescape( $yim ) . ", 
        " . $db->dbescape( $phone ) . ", " . $db->dbescape( $fax ) . ", " . $db->dbescape( $mobile ) . ", 1, '', 0, '', '')";
		$db->sql_query( $sql );
...
{
	$login = $pass = $re_pass = $full_name = $email = $website = $location = $yim = $phone = $fax = $mobile = "";
}
