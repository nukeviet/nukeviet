...
if (defined ( 'NV_IS_BANNER_CLIENT' )) {
	...
	if ($nv_Request->isset_request ( 'confirm', 'post' )) {
		...
		if (! empty ( $begintime ) and ! preg_match ( "/^([0-9]{1,2})\.([0-9]{1,2})\.([0-9]{4})$/", $begintime ))
			$begintime = "";
		if (! empty ( $endtime ) and ! preg_match ( "/^([0-9]{1,2})\.([0-9]{1,2})\.([0-9]{4})$/", $endtime ))
			$endtime = "";
		if ($url == "http://")
			$url = "";
		if (empty ( $title )) {
