...
if ( $nv_Request->isset_request( 'file', 'get' ) )
{
    ...
    $alias = $nv_Request->get_string( 'file', 'get', '' );

    if ( empty( $alias ) or ! preg_match( "/^([a-z0-9\-\_\.]+)$/i", $alias ) )
    {
    ...
    $query = "SELECT * FROM `" . NV_PREFIXLANG . "_" . $module_data . "` WHERE `alias`=" . $db->dbescape( $alias ) . " AND `catid` IN (" . $in . ") AND `status`=1";
    $result = $db->sql_query( $query );
    $numrows = $db->sql_numrows( $result );
    if ( $numrows != 1 )
    {
    ...
    $row['cattitle'] = "<a href=\"" . NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;cat=" . $list_cats[$row['catid']]['alias'] . "\">" . $list_cats[$row['catid']]['title'] . "</a>";
        ...
        if ( ! empty( $row['fileupload'] ) )
        {
            ...
            foreach ( $fileupload as $file )
            {
                if ( ! empty( $file ) )
                {
                    ...
                    {
                        $new_name = str_replace( "-", "_", $alias ) . ( $count_file > 1 ? "_part" . str_pad( $a, 2, '0', STR_PAD_LEFT ) : "" ) . "." . nv_getextension( $file );
                        $row['fileupload'][] = array( //
    ...
    else
    {
        ...
        {
            $row['download_info'] = sprintf( $lang_module['download_not_allow_info1'], NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users", NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&amp;" . NV_OP_VARIABLE . "=register" );
        }
    }
...
$cat = $nv_Request->get_string( 'cat', 'get', '' );
...
} elseif ( is_numeric( $cat ) )
{
    ...
    if ( isset( $list_cats[$cat] ) )
    {
        ...
        $in = "`catid`=" . $cat;
        $base_url .= "&amp;cat=" . $list_cats[$cat]['alias'];
    }
    else
    {
        Header( "Location: " . NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name );
...
{
    $cattitle = "<a href=\"" . NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;cat=" . $list_cats[$row['catid']]['alias'] . "\">" . $list_cats[$row['catid']]['title'] . "</a>";
    $more_link = NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;cat=" . $list_cats[$row['catid']]['alias'] . "&amp;file=" . $row['alias'];
...
if ( ! empty( $subcats ) )
{
    ...
    {
        $subs[] = array( //
            'title' => "<a href=\"" . NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;cat=" . $list_cats[$sub]['alias'] . "\">" . $list_cats[$sub]['title'] . "</a>", //
