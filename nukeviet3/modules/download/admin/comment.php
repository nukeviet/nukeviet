...
$contents .= "</tr>\n";
$contents .= "</tfoot>\n";
$sql = "SELECT a.tid, a.lid, a.comment, a.email, a.status, b.title  FROM `" . NV_PREFIXLANG . "_" . $module_data . "_comments` a INNER JOIN `" . NV_PREFIXLANG . "_" . $module_data . "` b ON a.lid=b.id";
$result = $db->sql_query ( $sql );
$num = $db->sql_numrows ( $result );
while ( list ( $cid, $id, $content, $email, $status, $title ) = $db->sql_fetchrow ( $result ) )
{
	$contents .= "<tbody>\n";
	$contents .= "<tr>\n";
	...
	$contents .= "<td><a href='".NV_BASE_SITEURL."?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=view&id=".$id."'>" . $title . "</a></td>\n";
	$contents .= "<td>" . $email . "</td>\n";
	$status = ($status == 1) ? '<a href="'.$cid.'" class="oneenable">'.$lang_module ['comment_enable'].'</a>' : '<a href="'.$cid.'" class="onedisable">'.$lang_module ['comment_disable'].'</a>';
