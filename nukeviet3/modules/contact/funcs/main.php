...
$array_rows = array();
$sql = "SELECT * FROM `" . NV_PREFIXLANG . "_" . $module_data . "_rows` WHERE `act`=1";
$result = $db->sql_query( $sql );
while ( $row = $db->sql_fetchrow( $result ) )
{
    $array_rows[$row['id']] = array(  //
...
if ( ! empty( $array_rows ) )
{
    ...
    if ( $checkss == md5( $client_info['session_id'] . $global_config['sitekey'] ) )
    {
        ...
        $ftitle = filter_text_input( 'ftitle', 'post', '', 1, 250 );
        $fcon = filter_text_textarea( 'fcon', '', NV_ALLOWED_HTML_TAGS );
        $fpart = $nv_Request->get_int( 'fpart', 'post', 0 );
