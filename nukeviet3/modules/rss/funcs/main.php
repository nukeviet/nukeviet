...
$result = $db->sql_query ( "SELECT module_file, custom_title FROM " . NV_MODULES_TABLE . " WHERE act=1 AND module_file!='rss' ORDER BY weight" );
while ( $row = $db->sql_fetchrow ( $result ) ) {
	$custom_title = $row ['custom_title'];
	$module_file = $row ['module_file'];
	if (file_exists ( NV_ROOTDIR . '/modules/' . $module_file . '/rssdata.php' )) {
		$contents .= $imgmid2 . "<a href=\"" . NV_BASE_SITEURL . "?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_file . "&" . NV_OP_VARIABLE . "=" . $module_name . "\">" . $iconrss . "</a>";
				...
				foreach ( $rssarray as $key => $value ) {
					if ($value ['parentid'] == 0) {
						...
						if (! empty ( $value ['numsubcat'] )) {
							...
							$contents .= getsubcat ( $array_catid, $module_file, $imgmid . $imgmid );
						}
					}
				}
				break;
...
function getsubcat($array_catid, $module_file, $i) {
	global $rssarray, $imgmid, $imgmid2, $iconrss, $module_name;
	$content = '';
	foreach ( $array_catid as $sub_catid ) {
		...
		if (! empty ( $rssarray [$sub_catid] ['numsubcat'] )) {
			...
			$content .= getsubcat ( $array_catid2, $module_file, $i . $imgmid );
		}
	}
	return $content;
}
