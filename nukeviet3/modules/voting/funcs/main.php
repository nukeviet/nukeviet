...
$difftimeout = 3600;
$vid = $nv_Request->get_int( 'vid', 'get', '' );
$checkss = $nv_Request->get_string( 'checkss', 'get', '' );
$lid = $nv_Request->get_string( 'lid', 'get', '' );
$note = "";
if ( $checkss == md5( $vid . session_id() . $global_config['sitekey'] ) and $vid > 0 and $lid != "" )
{
    ...
    $lang = array( 
        "total" => $lang_module['voting_total'], "counter" => $lang_module['voting_counter'], "publtime" => $lang_module['voting_pubtime'] 
    );
    $voting = array( 
        "total" => array_sum( $totalvote ), "pubtime" => $pubtime, "row" => $voting['row'], "lang" => $lang, "note" => $note 
    );
