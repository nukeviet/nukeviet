...
foreach ( $global_array_cat as $catid_i => $array_cat_i )
{
	...
	if ( $array_cat_i['parentid'] == 0 )
    {
        $array_cat[$catid_i] = array( 
            "title" => $array_cat_i['title'], "link" => $array_cat_i['link'], "description" => $array_cat_i['description'], "catimage" => $array_cat_i['catimage'], "subcat" => array() 
        );
        $sql = "SELECT `id` , `author` , `title` , `alias` , `url` , `urlimg` , `note` , `description` , `add_time` , `hits_total` 
    ...
    else
    {
        ...
        $array_cat[$parentid]['subcat'][] = array( 
            "title" => $global_array_cat[$catid_i]['title'], "link" => $global_array_cat[$catid_i]['link'], "count_link" => $global_array_cat[$catid_i]['count_link'] 
        );
    }
