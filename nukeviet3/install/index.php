...
{
    $array_dir = array( 
        NV_DATADIR, NV_SESSION_SAVE_PATH, NV_LOGS_DIR, NV_LOGS_DIR . "/data_logs", NV_LOGS_DIR . "/dump_backup", NV_LOGS_DIR . "/error_logs", NV_LOGS_DIR . "/error_logs/errors256", NV_LOGS_DIR . "/error_logs/old", NV_LOGS_DIR . "/error_logs/tmp", NV_LOGS_DIR . "/ip_logs", NV_LOGS_DIR . "/ref_logs", NV_CACHEDIR, NV_UPLOADS_DIR, NV_TEMP_DIR 
    );
        ...
        else
        {
            ...
            else
            {
                ...
                if ( empty( $db_config['error'] ) )
                {
                    ...
                    if ( ! file_exists( NV_ROOTDIR . "/install/data_" . NV_LANG_DATA . ".php" ) )
                    {
                        $filesavedata = "en";
                    }
                    ...
                    $result = $db->sql_query( "SELECT catid FROM `" . $db_config['prefix'] . "_" . NV_LANG_DATA . "_news_cat` ORDER BY `order` ASC" );
                    ...
                    $result = $db->sql_query( "SELECT id, listcatid FROM `" . $db_config['prefix'] . "_" . NV_LANG_DATA . "_news_rows` ORDER BY `id` ASC" );
                    while ( list( $id, $listcatid ) = $db->sql_fetchrow( $result ) )
                    {
                        $arr_catid = explode( ",", $listcatid );
                        foreach ( $arr_catid as $catid )
                        {
                            $db->sql_query( "INSERT INTO `" . $db_config['prefix'] . "_" . NV_LANG_DATA . "_news_" . $catid . "` SELECT * FROM `" . $db_config['prefix'] . "_" . NV_LANG_DATA . "_news_rows` WHERE `id`=" . $id . "" );
                        }
                    }
                    $db->sql_freeresult();
    ...
    if ( $nv_Request->isset_request( 'nv_login,nv_password', 'post' ) )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $sql = "INSERT INTO `" . $db_config['prefix'] . "_users` (`userid`, `username`, `password`, `email`, `full_name`, `gender`, `photo`, `birthday`, `sig`, `regdate`, `website`, `location`, `yim`, `telephone`, `fax`, `mobile`, `question`, `answer`, `passlostkey`, `view_mail`, `remember`, `in_groups`, `active`, `checknum`, `last_login`, `last_ip`, `last_agent`, `last_openid`) 
                VALUES(NULL, " . $db->dbescape( $login ) . ", " . $db->dbescape( $password ) . ", " . $db->dbescape( $email ) . ", " . $db->dbescape( $login ) . ", '', '', 0, NULL, " . NV_CURRENTTIME . ", '', '', '', '', '', '', '', '', '', 0, 1, '', 1, '', " . NV_CURRENTTIME . ", '', '', '')";
