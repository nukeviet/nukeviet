...
{
    global $module_name, $module_info, $module_file, $global_config, $lang_module, $module_name, $my_head;
...
function search_result_theme ( $key, $numRecord, $per_pages, $pages, $array_content, $url_link, $catid )
{
    ...
    if ( ! empty( $array_content ) )
    {
        foreach ( $array_content as $value )
        {
            ...
            //$xtpl->assign( 'SOURCE', empty(BoldKeywordInStr( GetSourceNews( $value['sourceid'] ), $key )) ? 'N/A' : '".BoldKeywordInStr( GetSourceNews( $value['sourceid'] ), $key )."');
