...
<div class="box-border">
    <div class="content-box">
        ...
        {CONTENT.bodytext}<!-- END: bodytext --><!-- BEGIN: error -->
        <div class="err">
            {CONTENT.error}
        </div>
        <!-- END: error --><!-- BEGIN: form -->
        <form id="fcontact" method="post" action="{ACTION_FILE}" onsubmit="return sendcontact('{NV_GFX_NUM}');">
            <div class="contact-form box-border content-box">
                ...
                <p class="rows">
                    &nbsp;{LANG.captcha}:&nbsp;<img height="22" src="{NV_BASE_SITEURL}index.php?scaptcha=captcha" title="{LANG.captcha}" alt="{LANG.captcha}" id="vimg" /><img alt="{CAPTCHA_REFRESH}" src="{CAPTCHA_REFR_SRC}" width="16" height="16" class="refresh" onclick="nv_change_captcha('vimg','fcode_iavim');"/><input type="text" maxlength="6" value="" id="fcode_iavim" name="fcode" class="input capcha" /><input type="hidden" name="checkss" value="{CHECKSS}" />&nbsp;<input type="button" value="{LANG.sendcontact}" id="btsend" name="btsend" class="button" />&nbsp; <input type="reset" value="{LANG.reset}" class="button-2" />
                </p>
            </div>
        </form><!-- END: form -->
    </div>
