...
function nv_get_submenu($module_file) {
	global $lang_global, $module_name;
	//ket noi voi file ngon ngu cua module
	$submenu = array ();
	if ($module_file != $module_name and file_exists ( NV_ROOTDIR . "/" . NV_ADMINDIR . "/modules/" . $module_file . "/functions.php" )) {
		...
		include (NV_ROOTDIR . "/" . NV_ADMINDIR . "/modules/" . $module_file . "/functions.php");
	}
	return $submenu;
}
		...
		if (! empty ( $v ['custom_title'] )) {
			...
			if (! empty ( $array_submenu )) {
				foreach ( $array_submenu as $mop => $submenu_i ) {
					...
					$xtpl->parse ( 'main.header.top_menu.top_menu_loop.submenu.submenu_loop' );
				}
				$xtpl->parse ( 'main.header.top_menu.top_menu_loop.submenu' );
			}
