    ...
    function nv_law_block_area($block_config)
    {
        ...
        foreach ($nv_laws_listarea as $cat) {
            if ($cat['parentid'] == 0) {
                ...
                $html .= "<li" . (!empty($cat['subcats']) ? ' class="hsub"' : '') . ">\n";
                $html .= "<a title=\"" . $cat['title'] . "\" href=\"" . $link . "\">" . nv_clean60($cat['title'], $title_length) . "</a>\n";

                if (!empty($cat['subcats'])) {
                    $html .= nv_content_subcat1($cat['subcats'], $title_length);
                }

                $html .= (!empty($cat['subcats']) ? '<span class="toggle-subs"><i class="fa fa-caret-down" aria-hidden="true"></i></span>' : '') . "</li>\n";
    ...
    function nv_content_subcat1($list_sub, $title_length)
    {
        ...
        if (empty($list_sub))
            ...
            foreach ($list_sub as $catid) {
                ...
                if (!empty($nv_laws_listarea[$catid]['subcats']))
                    $html .= nv_content_subcat1($nv_laws_listarea[$catid]['subcats'], $title_length);
                $html .= "</li>\n";
            }
            $html .= "</ul>\n";
            return $html;
...
if (defined('NV_SYSTEM')) {
    $content = nv_law_block_area($block_config);
}
