...
if (empty($all_page) and !$nv_Request->isset_request('add', 'get')) {
    ...
    if ($nv_Request->isset_request('add', 'get') or $nv_Request->isset_request('edit, id', 'get')) {
        ...
        if ($nv_Request->isset_request('save', 'post')) {
            ...
            if (empty($post['keywords']))
                $post['keywords'] = nv_get_keywords((!empty($post['bodytext']) ? $post['bodytext'] : $post['introtext']));
        ...
        if (!empty($post['introtext']))
            $post['introtext'] = nv_htmlspecialchars($post['introtext']);
        if (!empty($post['bodytext']))
            $post['bodytext'] = nv_htmlspecialchars($post['bodytext']);
        if (!empty($post['note']))
            $post['note'] = nv_htmlspecialchars($post['note']);
    ...
    $search = array('keywords' => '', 'cid' => 0, 'aid' => 0, 'sid' => 0);
