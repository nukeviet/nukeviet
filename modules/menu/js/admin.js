...
function nv_link(module) {
	...
	if (new_status != 0) {
		...
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=add_menu&nocache=' + new Date().getTime(), 'action=1&module=' + new_status, function(res) {
			$('#thu').html(res);
		});
	} else {
		$('input#link').val('');
...
function nv_link2(blog_menu) {
	...
	$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=add_menu&nocache=' + new Date().getTime(), 'item=1&mid=' + new_status, function(res) {
		$('#parentid').html(res);
	});
}
