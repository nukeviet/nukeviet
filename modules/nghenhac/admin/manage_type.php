...
if($nv_Request->get_title('them', 'post'))
{
    ...
    $sql = "INSERT INTO `nv4_vi_nghenhac_questions`(`cat_name`, `add_time`, `update_time`) VALUES (:tentheloai,:add_time,:update_time)";
    $sth = $db->prepare($sql);
...
} else if($nv_Request->get_title('sua', 'post'))
{
    ...
    $sql = "UPDATE `nv4_vi_nghenhac_questions` SET cat_name=:cat_name,update_time=:update_time WHERE id=:id";
    $sth = $db->prepare($sql);
...
} else if($nv_Request->get_title('delete', 'post'))
{
    ...
    $sql = "DELETE FROM `nv4_vi_nghenhac_questions` WHERE id=:id";
    $sth = $db->prepare($sql);
    $sth->bindParam("id",$id);
    $ext = $sth->execute();
    if($ext)
...
$query = "SELECT * FROM nv4_vi_nghenhac_questions";

$result = $db->query($query);
while ($row = $result->fetch()) {
    
    $xtpl->assign('rows',$row);
