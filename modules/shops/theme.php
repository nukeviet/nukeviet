 ...
 * @return
 */
function view_mb($data_content, $html_pages = '', $sort = 0) {//print_r($data_content);die();
	global $module_info, $lang_module, $module_file, $module_name, $pro_config, $op, $array_displays, $array_wishlist_id;

	$xtpl = new XTemplate('mb.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
	$xtpl -> assign('LANG', $lang_module);
	$xtpl -> assign('TEMPLATE', $module_info['template']);
	$xtpl -> assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
    ...
    { 
        if ($in_bike1["id_cd"] > $in_bike2["id_cd"]) 
        { 
            return 1; // để tăng dần nếu = -1 thì giảm dan
        } 
        else if ($in_bike1["id_cd"] == $in_bike2["id_cd"]) 
        { 
            return 0; 
        } 
        else 
    ...
    uasort($data_content, "compare_price"); 
//print_r($data_content);die;
		$id_cdmt_cur = 0;
		foreach ($data_content as $data_row) {
			//print_r($data_row);die;
			$xtpl -> assign('ID_MB', $data_row['product_code']);
			$xtpl -> assign('PHONGNGU', $data_row['phongngu']);
			
			$xtpl -> assign('ID_MN', $data_row['id_mn']);
			$xtpl -> assign('ID', $data_row['id']);
			
			if (!empty($data_row['otherimage'])) {
				$otherimage = explode('|', $data_row['otherimage']);//print_r($otherimage);die;
			} else {
				$otherimage = array();
			}
			...
			if (!empty($otherimage)) {
				foreach ($otherimage as $otherimage_i) {
					...
					if (!empty($otherimage_i) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $otherimage_i)) {
						...
						$xtpl -> assign('SRC', $otherimage_i);
						
						/*
						IF($id_cdmt_cur == 0){
													$xtpl -> assign('ID_MB1', $data_row['product_code']);
												}*/
						
						
						$xtpl -> parse('main.tab_content.img');
					}
			...
			// hien phan gia các gói phát sinh
			$xtpl -> assign('GOIKTCB', number_format($data_row['goiktcb']));
			$xtpl -> assign('GOIKTCB1', $data_row['goiktcb']);
			
			$xtpl -> assign('GOIBSKC', number_format($data_row['goibskc']));
			$xtpl -> assign('GOIBSXPXD', number_format($data_row['goibsxpxd']));
			$xtpl -> assign('GOICSBC', number_format($data_row['goicsbc']));
			$xtpl -> assign('MODULE_NAME', $module_name);
			
			$xtpl -> parse('main.tab_content');
			
			if ($data_row['id_cd'] > $id_cdmt_cur) {
				$xtpl -> assign('TITLE_CDMT', $data_row['title_cdmt']);
				$xtpl -> assign('CD', $data_row['title_cd']);
				$xtpl -> parse('main.nav_mb.clear');
				
				IF($id_cdmt_cur != 0){
					$xtpl -> parse('main.nav_mb.collapse');
				}
			}
			$id_cdmt_cur = $data_row['id_cd'];
			$xtpl -> parse('main.nav_mb');
		}
	}
 ...
 * @return
 */
function view_other($data_content, $html_pages = '', $sort = 0) {
	global $module_info, $lang_module, $module_file, $module_name, $pro_config, $op, $array_displays, $array_wishlist_id;

	$xtpl = new XTemplate('product_other.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
	$xtpl -> assign('LANG', $lang_module);
	$xtpl -> assign('TEMPLATE', $module_info['template']);
	$xtpl -> assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
	...
	if (!empty($data_content)) {
		...
		foreach ($data_content as $data_row) {//print_r($data_content);die;
			$xtpl -> assign('ID', $data_row['id']);
			...
			$xtpl -> assign('MAT_TIEN', $data_row['title_cdmt']);
			$xtpl -> parse('main.items.mat_tien');
			$xtpl -> assign('SO_TANG', $data_row['title_sotang']);
			$xtpl -> parse('main.items.so_tang');
...
function view_custom($array_custom, $template) {
	...
	//print_r($array_custom);die($template.NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file.'cat_form_'.$template.'.tpl');
 ...
 * @return
 */
function detail_product1($data_content, $data_unit, $data_shop, $data_others, $array_other_view) {
...
//print_r($data_content);die;
	...
	$xtpl = new XTemplate('detail1.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
	$xtpl -> assign('LANG', $lang_module);
	$xtpl -> assign('MODULE', $module_name);
	$xtpl -> assign('TEMPLATE', $module_info['template']);
	$xtpl -> assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
	...
	if (!empty($data_content)) {
		...
		$xtpl -> assign('MOTA', $lang_module['mota']);
		
		$xtpl -> assign('ID_MN', $data_content['id']);
		$xtpl -> assign('p', $price['price']);
		...
		if (!empty($data_content['array_custom'])) {
			//print_r($data_content['array_custom']);die;
		...
		if (!empty($otherimage)) {
			foreach ($otherimage as $otherimage_i) {
				if (!empty($otherimage_i) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $otherimage_i)) {
					...
					$xtpl -> parse('main.otherimage.img');
				}
			}
 ...
 * @return
 */
function detail_product($data_content, $data_unit, $data_shop, $data_others, $array_other_view, $array_mb) {
	global $module_info, $lang_module, $module_file, $module_name, $my_head, $pro_config, $global_config, $global_array_group, $array_wishlist_id, $client_info;
