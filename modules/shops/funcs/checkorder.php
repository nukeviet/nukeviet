...
if ( $checkss == md5( $user_info["userid"] . $global_config['sitekey'] . session_id() ) )
{
	...
	$result = $db->sql_query( $sql );
	while ( list( $order_id, $order_code, $payment, $payment_id, $payment_data ) = $db->sql_fetchrow( $result ) )
	{
		$array_data_payment[$payment]['data'][$order_code] = array( 
			'order_code' => $order_code, 'payment_id' => $payment_id, 'order_id' => $order_id, 'payment_data' => $payment_data 
		);
	}
