...
if ( $order_id > 0 and $checkss == md5( $order_id . $global_config['sitekey'] . session_id() ) )
{
    ...
    $data_order = $db->sql_fetchrow( $re );
    if ( $pro_config['active_order_number'] == '0' )
    {
        product_number_order( $data_order['listid'], $data_order['listnum'], "+" );
    }
    //*********** cong lai san pham trong kho/////////////////
    $db->sql_query( "DELETE FROM `" . $db_config['prefix'] . "_" . $module_data . "_orders` WHERE `order_id`=" . $order_id . " AND `transaction_status` < 1" );
    if ( $db->sql_affectedrows() )
    {
        $db->sql_query( "DELETE FROM `" . $db_config['prefix'] . "_" . $module_data . "_transaction` WHERE `order_id`=" . $order_id . "" );
