...
$sql = "SELECT id, listcatid, " . NV_LANG_DATA . "_alias, " . NV_LANG_DATA . "_title FROM `" . $db_config ['prefix'] . "_" . $module_data . "_rows` WHERE topic_id ='" . $topicid . "' ORDER BY `id` ASC";
$result = $db->sql_query( $sql );
...
if ( $num > 0 )
{
    ...
    while ( $row = $db->sql_fetchrow( $result ) )
    {
        ...
        $link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $global_array_cat[$catid_i]['alias'] . "/" . $row[NV_LANG_DATA.'_alias'] . "-" . $row['id'];
        
        $class = ( $a % 2 ) ? " class=\"second\"" : "";
        $contents .= "<tbody" . $class . ">\n";
        $contents .= "<tr>\n";
        $contents .= "<td><input type='checkbox' name='newsid' value='" . $row['id'] . "'/></td>\n";
        $contents .= "<td align=\"left\"><a target=\"_blank\" href=\"" . $link . "\">" . $row[NV_LANG_DATA . "_title"] . "</a></td>\n";
