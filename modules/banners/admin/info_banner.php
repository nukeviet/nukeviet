...
$contents['edit'] = array(
	NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=edit_banner&amp;id=" . $id,
	$lang_global['edit']
);
$contents['del'] = array(
	"nv_b_del2(" . $id . ");",
...
$contents['rows'][] = array(
	...
	"<a href=\"" . NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . NV_BANNER_DIR . "/" . $row['file_name'] . "\" rel=\"shadowbox;height=" . $row['height'] . ";width=" . $row['width'] . "\">" . $lang_module['click_show_img'] . "</a>"
);
if( ! empty( $row['imageforswf'] ) )
{
	$contents['rows'][] = array(
		$lang_module['imageforswf'],
		"<a href=\"" . NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . NV_BANNER_DIR . "/" . $row['imageforswf'] . "\" rel=\"shadowbox;height=" . $row['height'] . ";width=" . $row['width'] . "\">" . $lang_module['click_show_img'] . "</a>"
	);
}
$contents['rows'][] = array(
	$lang_module['img_info1'],
	$img_info
);
$contents['rows'][] = array(
...
$contents['rows'][] = array(
	$lang_module['add_date'],
	date( "d/m/Y H:i", $row['add_time'] )
);
$contents['rows'][] = array(
	$lang_module['publ_date'],
	date( "d/m/Y H:i", $row['publ_time'] )
);
$contents['rows'][] = array(
	$lang_module['exp_date'],
	( ! empty( $row['exp_time'] ) ? date( "d/m/Y H:i", $row['exp_time'] ) : $lang_module['unlimited'])
);
$contents['rows'][] = array(
	$lang_global['status'],
	$lang_module['act' . $row['act']]
);
$contents['rows'][] = array(
	$lang_module['hits_total'],
	$row['hits_total']
);
