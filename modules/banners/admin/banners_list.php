...
if (isset($clients[$contents['clid']])) {
    ...
    $contents['caption'] .= " " . sprintf($nv_Lang->getModule('banners_list_cl'), $clients[$contents['clid']]);
} elseif (isset($plans[$contents['pid']])) {
    $sql .= " AND pid=" . $contents['pid'];
    $contents['caption'] .= " " . sprintf($nv_Lang->getModule('banners_list_pl'), $plans[$contents['pid']]);
}
