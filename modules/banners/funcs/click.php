...
if( $id > 0 )
{
	list( $id, $click_url ) = $db->sql_fetchrow( $db->sql_query( "SELECT id, click_url FROM `" . NV_BANNERS_GLOBALTABLE. "_rows` WHERE id='$id' AND act='1'" ) );
	...
	if( $id > 0 and ! empty( $click_url ) )
	{
		...
		if( $time_set == 0 )
		{
			...
			$db->sql_query( "UPDATE `" . NV_BANNERS_GLOBALTABLE. "_rows` SET hits_total=hits_total+1 WHERE id='" . $id . "'" );
