...
if (! defined('NV_IS_BANNER_CLIENT') or empty($banner_client_info)) {
    die('&nbsp;');
}
...
if ($nv_Request->get_int('save', 'post') == '1') {
    ...
    if (! empty($check_email)) {
        die(strip_tags($check_email) . '|email_iavim');
    } elseif (! empty($pass) and ! empty($check_pass)) {
        die(strip_tags($check_pass) . '|pass_iavim');
    } elseif (! empty($pass) and empty($re_pass)) {
        die(strip_tags($lang_global['re_password_empty']) . '|pass_iavim');
    } elseif (! empty($pass) and $pass != $re_pass) {
        die($lang_global['passwordsincorrect'] . '|pass_iavim');
    } elseif (empty($full_name)) {
        die(strip_tags($lang_module['full_name_empty']) . '|full_name');
    } elseif (! empty($website) and ! nv_is_url($website)) {
        die(strip_tags($lang_module['website_incorrect']) . '|website_iavim');
    } elseif (! empty($yim) and ! preg_match('/^[a-zA-Z0-9\.\-\_]+$/', $yim)) {
        die(strip_tags($lang_module['yim_incorrect']) . '|yim_iavim');
    }
    ...
    if ($stmt->fetchColumn()) {
        die(strip_tags(sprintf($lang_module['email_is_already_in_use'], $email)) . '|email_iavim');
    }
    ...
    $stmt = $db->prepare($sql) ;
    if (! empty($pass)) {
        $stmt->bindParam(':pass', $crypt->hash_password($pass, $global_config['hashprefix']), PDO::PARAM_STR);
    }
