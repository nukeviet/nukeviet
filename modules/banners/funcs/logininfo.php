	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				$stmt->bindParam(':check_num', $check_num, PDO::PARAM_STR, strlen($check_num));
				$stmt->bindParam(':last_ip', $client_info['ip'], PDO::PARAM_STR, strlen($client_info['ip']));
				$stmt->bindParam(':last_agent', $agent, PDO::PARAM_STR, strlen($agent));
