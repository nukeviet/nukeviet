...
function nv_b_list_theme($contents, $array_users = [])
{
    ...
    if (!empty($contents['rows'])) {
        foreach ($contents['rows'] as $b_id => $values) {
            $values['delfile'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=del_banner&id=' . $b_id . '&checkss=' . md5($b_id . NV_CHECK_SESSION);
