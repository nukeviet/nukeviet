...
foreach ( $topicList as $__k => $__v )
{
	if ( $__k != $topic['id'] )
	{
		if ( $__v['parentid'] == $topic['id'] ) $_otherTopic['sub'][] = $topicList[$__k];
		elseif ( $__v['parentid'] == $topic['parentid'] ) $_otherTopic['main'][] = $topicList[$__k];
	}
}
...
$sql = "SELECT SQL_CALC_FOUND_ROWS a.*,b.view FROM `" . NV_PREFIXLANG . "_" . $module_data . "_clip` a, 
    ...
    WHERE a.tid=" . $db->dbescape( $topic['id'] ) . " 
    AND a.id=b.cid 
    AND a.status=1 
    ORDER BY a.id DESC 
