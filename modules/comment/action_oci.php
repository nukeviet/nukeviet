...
$query = $db->query( "select table_name from all_tables WHERE table_name = '" . strtoupper( $db_config['prefix'] . "_" . $lang . "_comment" ) . "'" );
while( $row = $query->fetch() )
{
	$sql_drop_module[] = 'drop table ' . $row['table_name'] . ' cascade constraints PURGE';
}

$query = $db->query( "select sequence_name from user_sequences WHERE sequence_name = '" . strtoupper( "SNV_" . $lang . "_comment" ) . "'" );
while( $row = $query->fetch() )
{
...
$sql_create_module[] = 'create sequence SNV_' . strtoupper( $lang . '_comment' ) . '_CMEN';

$sql_create_module[] = 'CREATE OR REPLACE TRIGGER TNV_' . strtoupper( $lang . '_comment' ) . '_CMEN
 BEFORE INSERT ON ' . $db_config['prefix'] . '_' . $lang . '_comment
...
$sql_create_module[] = "CREATE INDEX inv_" . $lang . "_cid ON " . $db_config['prefix'] . "_" . $lang . "_comment(module,area,id) TABLESPACE USERS";
$sql_create_module[] = "CREATE INDEX inv_" . $lang . "_cposttime ON " . $db_config['prefix'] . "_" . $lang . "_comment(post_time) TABLESPACE USERS";