...
$sql = 'SELECT listcatid FROM ' . NV_PREFIXLANG . '_' . $m_values['module_data'] . '_rows WHERE id=' . $row['id'];
...
$query = 'UPDATE ' . NV_PREFIXLANG . '_' . $m_values['module_data'] . '_rows SET hitscm=' . $numf . ' WHERE id=' . $row['id'];
$db->query( $query );
foreach( $array_catid as $catid_i )
{
	$query = 'UPDATE ' . NV_PREFIXLANG . '_' . $m_values['module_data'] . '_' . $catid_i . ' SET hitscm=' . $numf . ' WHERE id=' . $row['id'];
	$db->query( $query );
}
