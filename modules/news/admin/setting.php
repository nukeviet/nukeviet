...
if (!empty($savesetting)) {
	...
	$array_config['use_elas'] = $nv_Request -> get_int('use_elas', 'post', 0);
...
$array_tooltip_position = array('top' => $lang_module['showtooltip_position_top'], 'bottom' => $lang_module['showtooltip_position_bottom'], 'left' => $lang_module['showtooltip_position_left'], 'right' => $lang_module['showtooltip_position_right']);

// Vi tri hien thi tooltip
foreach ($array_tooltip_position as $key => $val) {
	$xtpl -> assign('TOOLTIP_P', array('key' => $key, 'title' => $val, 'selected' => $key == $module_config[$module_name]['tooltip_position'] ? ' selected="selected"' : ''));
...
for ($i = 0; $i <= 6; ++$i) {
	$xtpl -> assign('RATING_POINT', array('key' => $i, 'title' => ($i == 6) ? $lang_module['no_allowed_rating'] : $i, "selected" => $i == $module_config[$module_name]['allowed_rating_point'] ? " selected=\"selected\"" : ""));
...
$xtpl -> assign('USE_ELAS', $module_config[$module_name]['use_elas'] ? ' checked="checked"' : '');
...
$array_config_source = array(0 => $lang_module['config_source_title'], 3 => $lang_module['config_source_link'], 1 => $lang_module['config_source_link_nofollow'], 2 => $lang_module['config_source_logo']);
...
if (defined('NV_IS_ADMIN_FULL_MODULE') or !in_array('admins', $allow_func)) {
	...
	while (list($group_id, $addcontent, $postcontent, $editcontent, $delcontent) = $result -> fetch(3)) {
		if (isset($groups_list[$group_id])) {
			$array_post_data[$group_id] = array('group_id' => $group_id, 'addcontent' => $addcontent, 'postcontent' => $postcontent, 'editcontent' => $editcontent, 'delcontent' => $delcontent);
		} else {
	...
	foreach ($groups_list as $group_id => $group_title) {
		...
		$xtpl -> assign('ROW', array('group_id' => $group_id, 'group_title' => $group_title, 'addcontent' => $addcontent ? ' checked="checked"' : '', 'postcontent' => $postcontent ? ' checked="checked"' : '', 'editcontent' => $editcontent ? ' checked="checked"' : '', 'delcontent' => $delcontent ? ' checked="checked"' : ''));
