...
if ($nv_Request->isset_request('savecat', 'post')) {
    ...
    if (!empty($tid)) {
        ...
        if (!$num) {
            nv_jsonOutput([
                'status' => 'error',
                'input' => 'tid',
                'message' => $lang_module['error_tag_tid']
            ]);
        }
    }
    ...
    if (nv_strlen($keywords) < 2) {
        nv_jsonOutput([
            'status' => 'error',
            'input' => 'keywords',
            'message' => $lang_module['error_tag_keywords']
        ]);
    }
    $dbexist = false;
    $keywords = setTagKeywords($keywords);
    ...
    if ($dbexist) {
        nv_jsonOutput([
            'status' => 'error',
            'input' => 'keywords',
            'message' => $lang_module['error_tag_keywords_exist']
        ]);
    }
    ...
    if (nv_strlen($title) < 2) {
        nv_jsonOutput([
            'status' => 'error',
            'input' => 'title',
            'message' => $lang_module['error_tag_title']
        ]);
    }
...
if ($incomplete) {
    ...
    $caption = $lang_module['tags_incomplete_link'];
} elseif ($complete) {
    $xtpl->parse('main.complete_link');
    $caption = $lang_module['tags_complete_link'];
} else {
    $xtpl->parse('main.all_link');
    $caption = $lang_module['tags_all_link'];
}
