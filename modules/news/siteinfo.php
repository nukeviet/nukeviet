...
// Tong so bai viet
list( $number ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM " . NV_PREFIXLANG . "_" . $mod_data . "_rows WHERE status= 1 AND publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ")" ) );
if( $number > 0 )
{
...
list( $number ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM " . NV_PREFIXLANG . "_" . $mod_data . "_rows WHERE admin_id NOT IN (" . implode( ',', $admins_module ) . ") " ) );
if( $number > 0 )
{
...
list( $number ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM " . NV_PREFIXLANG . "_" . $mod_data . "_rows WHERE exptime > 0 AND exptime<" . NV_CURRENTTIME . "" ) );
if( $number > 0 )
{
...
list( $number ) = $db->sql_fetchrow( $db->sql_query( "SELECT COUNT(*) FROM " . NV_PREFIXLANG . "_" . $mod_data . "_rows WHERE status = 1 AND exptime>" . NV_CURRENTTIME . "" ) );
if( $number > 0 )
{
