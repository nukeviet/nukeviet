...
if (! empty($catid)) {
    ...
    $db_slave->from(NV_PREFIXLANG . '_' . $module_data . '_' . $catid)
        ->where('status=1 AND instant_active=1' . ($gettime ? ' AND (publtime>= ' . $gettime . ' OR edittime >= ' . $gettime . ')' : ''));
} else {
    $db_slave->from(NV_PREFIXLANG . '_' . $module_data . '_rows')
        ->where('status=1 AND inhome=1 AND instant_active=1' . ($gettime ? ' AND (publtime>= ' . $gettime . ' OR edittime >= ' . $gettime . ')' : ''));
}
