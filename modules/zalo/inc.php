...
/**
 * Gửi tin nhắn tương tác dạng text
 ...
 * @throws PDOException
 */
function sent_textlist_message($template_id, $user_id, $message_id)
{
    global $db, $global_config;

    $is_localhost = is_localhost();

    $sql = 'SELECT * FROM ' . NV_MOD_TABLE . '_template WHERE id=' . $template_id . " AND type='text'";
    $result = $db->query($sql);
    $row = $result->fetch();
    if (!$row) {
        return false;
    ...
    $elements = [];
    $elements0 = array_shift($content);
    if (!nv_is_url($elements0['image_url'])) {
        if ($is_localhost) {
            return false;
        }

        if (file_exists(NV_ROOTDIR . '/' . NV_UPLOADS_DIR . '/zalo/' . $elements0['image_url'])) {
            $elements0['image_url'] = NV_MY_DOMAIN . NV_BASE_SITEURL . NV_UPLOADS_DIR . '/zalo/' . $elements0['image_url'];
        } else {
            $elements0['image_url'] = NV_MY_DOMAIN . $elements0['image_url'];
        }
    }
    $elements[0] = [
    ...
    foreach ($content as $list) {
        if (!empty($list['title']) and !empty($list['image_url']) and !empty($list['default_action'])) {
            ...
            if (!nv_is_url($list['image_url'])) {
                ...
                if (file_exists(NV_ROOTDIR . '/' . NV_UPLOADS_DIR . '/zalo/' . $list['image_url'])) {
                    $list['image_url'] = NV_MY_DOMAIN . NV_BASE_SITEURL . NV_UPLOADS_DIR . '/zalo/' . $list['image_url'];
                } else {
                    $list['image_url'] = NV_MY_DOMAIN . $list['image_url'];
                }
            }
...
/**
 * Gửi tin nhắn tương tác dạng nút
 ...
 * @throws PDOException
 */
function sent_btnlist_message($template_id, $user_id, $message_id)
{
    global $db, $global_config;

    $sql = 'SELECT * FROM ' . NV_MOD_TABLE . '_template WHERE id=' . $template_id . " AND type='btn'";
    $result = $db->query($sql);
    $row = $result->fetch();
    if (!$row) {
        return false;
    ...
    $buttons = [];
    foreach ($content['buttons'] as $list) {
