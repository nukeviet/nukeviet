...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        if (empty($dataform['fid'])) {
            $_columns_array = $db->columns_array($db_config['prefix'] . '_' . $module_data);
            ...
            if ($dataform['max_length'] <= 4294967296 and ! empty($dataform['field']) and ! empty($dataform['title']) and !isset($_columns_array[$dataform['field']])) {
                ...
                if ($dataform['fid']) {
                    ...
                    $save = $db->exec("ALTER TABLE " . $db_config['prefix'] . "_" . $module_data . "_info ADD " . $dataform['field'] . " " . $type_date);
                }
            }
        } elseif ($dataform['max_length'] <= 4294967296) {
            ...
            if ($save and $dataform['max_length'] != $dataform_old['max_length']) {
                ...
                $save = $db->exec("ALTER TABLE " . $db_config['prefix'] . "_" . $module_data . "_info CHANGE " . $dataform_old['field'] . " " . $dataform_old['field'] . " " . $type_date);
            }
        }
        if ($save) {
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    list($fid, $field, $weight) = $db->query('SELECT fid, field, weight FROM ' . $db_config['prefix'] . '_' . $module_data . '_field WHERE fid=' . $fid)->fetch(3);

    if ($fid and ! empty($field)) {
        $query1 = 'DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_field WHERE fid=' . $fid;
        $query2 = 'ALTER TABLE ' . $db_config['prefix'] . '_' . $module_data . '_info DROP ' . $field;
