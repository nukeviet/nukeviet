...
if ($nv_Request->isset_request('confirm', 'post')) {
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . ' WHERE email= :email');
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_reg WHERE email= :email');
    $stmt->bindParam(':email', $_user['email'], PDO::PARAM_STR);
    $stmt->execute();
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_openid WHERE email= :email');
    $stmt->bindParam(':email', $_user['email'], PDO::PARAM_STR);
    $stmt->execute();
    ...
    if (! empty($_user['photo'])) {
        ...
        if (! empty($_user['photo'])) {
            $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . ' SET photo= :file_name WHERE userid=' . $userid);
    ...
    $db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET numbers = numbers+1 WHERE group_id=' . ($_user['is_official'] ? 4 : 7));
