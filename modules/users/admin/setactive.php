...
$sql = 'SELECT a.lev, b.username, b.active, b.idsite FROM ' . NV_AUTHORS_GLOBALTABLE . ' a, ' . $db_config['prefix'] . '_' . $module_data . ' b WHERE a.admin_id=' . $userid . ' AND a.admin_id=b.userid';
$row = $db->query($sql)->fetch(3);
if (empty($row)) {
    $level = 0;
    $sql = 'SELECT username, active, idsite FROM ' . $db_config['prefix'] . '_' . $module_data . ' WHERE userid=' . $userid;
...
if (empty($level) or $admin_info['level'] < $level) {
    ...
    $active = $active ? 0 : 1;
    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . ' SET active=' . $active . ' WHERE userid=' . $userid;
    $result = $db->query($sql);
