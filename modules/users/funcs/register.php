...
function nv_check_username_reg($login)
{
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . ' WHERE md5username= :md5username');
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_reg WHERE md5username= :md5username');
    $stmt->bindValue(':md5username', nv_md5safe($login), PDO::PARAM_STR);
    $stmt->execute();
    if ($stmt->fetchColumn()) {
        return sprintf($lang_module['account_registered_name'], $login);
...
function nv_check_email_reg($email)
{
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . ' WHERE email RLIKE :pattern');
    ...
    $stmt = $db->prepare('SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_openid WHERE email RLIKE :pattern');
    $stmt->bindParam(':pattern', $pattern, PDO::PARAM_STR);
    $stmt->execute();
    if ($stmt->fetchColumn()) {
        return sprintf($lang_module['email_registered_name'], $email);
...
if ($checkss == $array_register['checkss']) {
    ...
    if ($global_config['allowuserreg'] == 2 or $global_config['allowuserreg'] == 3) {
        $sql = "INSERT INTO " . $db_config['prefix'] . "_" . $module_data . "_reg (username, md5username, password, email, first_name, last_name, regdate, question, answer, checknum, users_info) VALUES (
			:username,
			:md5username,
			:password,
			:email,
...
if ($nv_Request->isset_request('get_usage_terms', 'post')) {
    $sql = "SELECT content FROM " . $db_config['prefix'] . "_" . $module_data . "_config WHERE config='siteterms_" . NV_LANG_DATA . "'";
