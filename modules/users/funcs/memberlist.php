...
if ($global_config['whoviewuser'] == 2 and ! defined('NV_IS_ADMIN')) {
    ...
    if (isset($array_op[1]) && ! empty($array_op[1])) {
        ...
        if (! empty($md5)) {
            $stmt = $db->prepare('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . ' WHERE md5username = :md5');
            ...
            if (! empty($item)) {
                ...
                $sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_info WHERE userid=' . $item['userid'];
                $result = $db->query($sql);
