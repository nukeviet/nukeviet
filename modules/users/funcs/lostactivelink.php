        ...
        if (! empty($data['userField'])) {
            ...
            if (! empty($check_email) and ! empty($check_login)) {
                ...
                if (empty($check_email)) {
                    $sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_reg WHERE email= :userField AND regdate>' . $exp;
                } else {
                    $sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_reg WHERE username= :userField AND regdate>' . $exp;
                }
                $stmt = $db->prepare($sql) ;
                    ...
                    if ($data['send']) {
                        if ($data['answer'] == $row['answer']) {
                            ...
                            if ($ok) {
                                ...
                                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_reg SET password= :password, checknum= :checknum WHERE userid=' . $row['userid']);
