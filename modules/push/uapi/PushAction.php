...
/**
 * NukeViet\Module\push\uapi\PushAction
 * API dùng để Thay đổi trạng thái của thông báo đẩy: đã xem/chưa xem, đã ẩn/chưa ẩn, yêu thích/Hủy yêu thích
 *
 ...
 * @access public
 */
class PushAction implements UiApi
{
    private $result;
    ...
    public function execute()
    {
        ...
        if (empty($row['shown_time']) and empty($row['viewed_time']) and empty($row['favorite_time']) and empty($row['hidden_time'])) {
            $db->query('INSERT IGNORE INTO ' . NV_PUSH_STATUS_GLOBALTABLE . ' (pid, userid, ' . $field_name . ') VALUES (' . $id . ', ' . $user_id . ', ' . $field_value . ')');
        } else {
            $db->query('UPDATE ' . NV_PUSH_STATUS_GLOBALTABLE . ' SET ' . $field_name . ' = ' . $field_value . ' WHERE pid=' . $id . ' AND userid=' . $user_id);
        }
