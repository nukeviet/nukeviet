...
if( $nv_Request->isset_request( 'add', 'get' ) )
{
	...
	if( $nv_Request->isset_request( 'submit', 'post' ) )
	{
		...
		else
		{
			...
			if( ! $is_error )
			{
				$stmt = $db->prepare ( 'SELECT COUNT(*) AS count FROM ' . NV_PREFIXLANG . '_' . $module_data . '_categories WHERE alias= :alias' );
				$stmt->bindParam(':alias', $array['alias'], PDO::PARAM_STR, strlen($rowcontent['topictext']));
				$stmt->execute();
...
if( $nv_Request->isset_request( 'edit', 'get' ) )
{
	...
	if( $nv_Request->isset_request( 'submit', 'post' ) )
	{
		...
		else
		{
			...
			if( ! $is_error )
			{
				$stmt = $db->prepare ('SELECT COUNT(*) AS count FROM ' . NV_PREFIXLANG . '_' . $module_data . '_categories WHERE id!=' . $catid . ' AND alias= :alias' );
				$stmt->bindParam(':alias', $array['alias'], PDO::PARAM_STR, strlen($array['alias']));
				$stmt->execute();
