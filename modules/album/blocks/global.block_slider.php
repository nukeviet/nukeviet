...
if (!nv_function_exists('nv_block_news_groups')) {
    ...
    function nv_block_news_groups($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_slider.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_slider.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
