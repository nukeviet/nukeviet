...
if( ! empty( $contact_allowed['view'] ) )
{
	...
	if( $num_items )
	{
		...
		{
			$image = array( NV_BASE_SITEURL . 'assets/images/mail_new.gif', 12, 9 );
			$status = 'New';
			$style = " style=\"font-weight:bold;cursor:pointer;white-space:nowrap;\"";

			if( $row['is_read'] == 1 )
			{
				$image = array( NV_BASE_SITEURL . 'assets/images/mail_old.gif', 12, 11 );
			...
			if( $row['is_reply'] )
			{
				$image = array( NV_BASE_SITEURL . 'assets/images/mail_reply.gif', 13, 14 );
