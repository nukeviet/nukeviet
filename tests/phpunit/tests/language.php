...
class TestsLanguageBase extends TestCase
{
    ...
    /**
     * Kiểm tra ngôn ngữ bị thừa so với Tiếng Việt
     */
        ...
        foreach ($files as $file) {
            ...
            foreach ($langs as $lang) {
                if (file_exists(NV_ROOTDIR . '/includes/language/' . $lang . '/' . $file)) {
                    ...
                    $this->assertCount(0, $redundant, 'Redundant:' . PHP_EOL . implode(PHP_EOL, array_keys($redundant)));
                    $this->assertCount(0, $notTranslated, 'Not Translated:' . PHP_EOL . implode(PHP_EOL, array_keys($notTranslated)));
                }
            }
        }
    }
    ...
    /**
     * Kiểm tra ngôn ngữ install không sử dụng
     */
    public function testLangInstallNotUse()
    {
        $langnotuse = [];
        $this->assertLessThanOrEqual(0, sizeof($langnotuse));
    }
}
