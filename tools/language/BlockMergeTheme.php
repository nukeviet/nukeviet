...
// Quét các giao diện cần xử lý
$array_themes = [];
$__tmp = scandir(NV_ROOTDIR . '/themes');
foreach ($__tmp as $__tmp__) {
    if (preg_match('/^[a-zA-Z]+[a-zA-Z0-9\_]{0,}$/', $__tmp__)) {
        $array_themes[] = $__tmp__;
    }
}

foreach ($array_themes as $theme_dir) {
    // Quét tiếp các block của theme
    $array_blocks = [];
    if (file_exists(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks')) {
        $__tmp = scandir(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks');
        foreach ($__tmp as $__tmp__) {
    ...
    foreach ($array_blocks as $block_info) {
        ...
        foreach ($array_static_lang as $lang) {
            ...
            if (file_exists(NV_ROOTDIR . '/themes/' . $theme_dir . '/language/' . $lang . '.php')) {
                $lang_translator = $lang_module = [];
                require NV_ROOTDIR . '/themes/' . $theme_dir . '/language/' . $lang . '.php';
                $finalLangModule = $lang_module;
            }
            $finalLangTranslator['author'] = 'VINADES.,JSC <contact@vinades.vn>';
            $finalLangTranslator['createdate'] = '04/03/2010, 15:22';
            $finalLangTranslator['copyright'] = '@Copyright (C) 2012 VINADES.,JSC. All rights reserved';
            $finalLangTranslator['info'] = '';
            $finalLangTranslator['langtype'] = 'lang_module';

            // Lang mới
            $finalLangModuleNew = $finalLangModule;

            /*
             * Đọc file ini lấy lang
             */
            $lang_translator = $lang_block = [];
            if (empty($lang_block) and file_exists(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['name'] . '.ini')) {
                // Đọc nội dung file ini tìm ra ngôn ngữ
                $xml = simplexml_load_file(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['name'] . '.ini');
            ...
            $lang_block_convert = [];
            if (empty($lang_block)) {
                echo $lang . ': Pass block ' . $theme_dir . ':' . $block_info['php_file'] . PHP_EOL;
            } else {
                foreach ($lang_block as $_key => $_value) {
                    if (!isset($finalLangModuleNew[$_key])) {
                        ...
                        if (isset($finalLangModuleNew[$_key_new]) and $finalLangModuleNew[$_key_new] != $_value) {
                            exit('Error key exists ' . $_key_new . ' lang ' . $lang . ' module ' . $theme_dir . ' block ' . $block_info['php_file']);
                        }
            ...
            if ($finalLangModuleNew != $finalLangModule) {
                ...
                file_put_contents(NV_ROOTDIR . '/themes/' . $theme_dir . '/language/' . $lang . '.php', $content_lang, LOCK_EX);
            }

            // Duyệt lang block build lại file php
            $php_file_contents = file_get_contents(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['php_file']);
            ...
            if ($php_file_contents_new != $php_file_contents) {
                file_put_contents(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['php_file'], $php_file_contents_new, LOCK_EX);
            }
        }

        // Đọc lại file ini và xóa phần config lang đi
        if (file_exists(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['name'] . '.ini')) {
            $file_ini_contents = file_get_contents(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['name'] . '.ini');
            $file_ini_contents_new = preg_replace('/[\r\n\s\t]*\<language\>(.*?)\<\/language\>[\r\n\s\t]*/isu', "\n", $file_ini_contents);
            if ($file_ini_contents_new != $file_ini_contents) {
                file_put_contents(NV_ROOTDIR . '/themes/' . $theme_dir . '/blocks/' . $block_info['name'] . '.ini', $file_ini_contents_new, LOCK_EX);
            }
        }
    }
}
