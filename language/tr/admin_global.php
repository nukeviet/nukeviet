<?php

/**
* @Project NUKEVIET 3.x
* @Author VINADES.,JSC (contact@vinades.vn)
* @Copyright (C) 2012 VINADES.,JSC. All rights reserved
* @Language Türkçe
* @Createdate Oct 25, 2012, 03:11:41 PM
*/

if( ! defined( 'NV_ADMIN' ) or ! defined( 'NV_MAINFILE' ) ) die( 'Stop!!!' );

$lang_translator['author'] = 'Nukevietdestek.com (destek@nukevietdestek.com)';
$lang_translator['createdate'] = '14/06/2011/03:30';
$lang_translator['copyright'] = '@Copyright (C) 2011 Nukevietdestek.com tüm hakları saklıdır';
$lang_translator['info'] = 'Dil Sevimsiz tarafından tercüme edildi';
$lang_translator['langtype'] = 'lang_global';

$lang_global['mod_authors'] = 'Yönetici';
$lang_global['mod_groups'] = 'Grup';
$lang_global['mod_database'] = 'Veritabanı';
$lang_global['mod_settings'] = 'Yapılandırma';
$lang_global['mod_cronjobs'] = 'Otomatik işlem';
$lang_global['mod_modules'] = 'Modüller';
$lang_global['mod_themes'] = 'Temalar';
$lang_global['mod_siteinfo'] = 'Bilgi';
$lang_global['mod_language'] = 'Dil';
$lang_global['mod_upload'] = 'Yükle';
$lang_global['mod_webtools'] = 'Web Araçları';
$lang_global['go_clientsector'] = 'Anasayfa';
$lang_global['go_clientmod'] = 'Web site';
$lang_global['please_select'] = 'Lütfen seçin';
$lang_global['admin_password_empty'] = 'Yönetici parolası ilan edilmemiştir';
$lang_global['adminpassincorrect'] = 'yönetici parolası &ldquo;<strong>%s</strong>&rdquo; yanlıştır. tekrar deneyin';
$lang_global['admin_password'] = 'Şifre';
$lang_global['admin_no_allow_func'] = 'Bu fonksiyon erişemiyor';
$lang_global['who_view'] = 'sağ görüntüle';
$lang_global['who_view0'] = 'Herkes';
$lang_global['who_view1'] = 'Kayıtlı Kullanıcılar';
$lang_global['who_view2'] = 'Yöneticiler';
$lang_global['who_view3'] = 'Kullanıcı Grupları';
$lang_global['level1'] = 'Süper';
$lang_global['level2'] = 'Gelen Yönetici';
$lang_global['level3'] = 'Modül Yönetimi';
$lang_global['groups_view'] = 'Grup okundu';
$lang_global['block_modules'] = 'Modüllerinde Blok';
$lang_global['hello_admin1'] = 'Hoşgeldin %1$s ! Son giriş tarihi: %2$s';
$lang_global['hello_admin2'] = 'Hesabınız: %1$s ! Bu yönetim kaydediliyor, %2$s';
$lang_global['hello_admin3'] = 'Hoşgeldiniz %1$s. This is the first time to login administration';
$lang_global['ftp_error_account'] = 'Hata: FTP sunucusuna bağlanılamıyor, FTP yapılandırmayı kontrol edin';
$lang_global['ftp_error_path'] = 'Error: Wrong configuration in Remote path';
$lang_global['login_error_account'] = 'Hata: Geçersiz kullanıcı adı. (Sadece harf, rakam ve Latin alfabesi. en az %1$s karakter, en fazla %1$s karakter)';
$lang_global['login_error_password'] = 'Hata: Girilen şifre geçersiz. (Sadece harf, rakam ve Latin alfabesi en az %1$s karakter, en fazla %1$s karakter)';
$lang_global['login_error_security'] = 'Hata: geçersiz Güvenlik Kodu! (Sadece Latin alfabesi %1$s karakter)';
$lang_global['error_zlib_support'] = 'Hata: Sunucu zlib uzantı desteklemez, Sen zlib uzatma etkinleştirmek için barındırma sağlayıcınıza başvurmanız gerekir.';
$lang_global['error_zip_extension'] = 'Hata: Sunucu ZIP uzantılı desteklemiyor, Sen ZIP uzantılı etkinleştirmek için barındırma sağlayıcınıza başvurmanız gerekir.';
$lang_global['error_uploadNameEmpty'] = 'UserFile Adı boş';
$lang_global['error_uploadSizeEmpty'] = 'UserFile boyutu boş';
$lang_global['error_upload_ini_size'] = 'Yüklenen dosya php.ini içindeki upload_max_filesize yönergesini aşıyor';
$lang_global['error_upload_form_size'] = 'Yüklenen dosya HTML formunda belirtilen MAX_FILE_SIZE aşıyor';
$lang_global['error_upload_partial'] = 'Yüklenen dosya sadece kısmen yüklendi';
$lang_global['error_upload_no_file'] = 'Hiçbir dosya yüklenmedi';
$lang_global['error_upload_no_tmp_dir'] = 'Geçici bir klasör Eksik';
$lang_global['error_upload_cant_write'] = 'Diske dosya yazma başarısız';
$lang_global['error_upload_extension'] = 'File upload stopped by extension';
$lang_global['error_upload_unknown'] = 'Bilinmeyen yükleme hatası';
$lang_global['error_upload_type_not_allowed'] = 'Bu dosya türüne izin verilmez';
$lang_global['error_upload_mime_not_recognize'] = 'Sisteme yüklenen dosyanın uzantı türünü tanımıyor';
$lang_global['error_upload_max_user_size'] = 'Dosya izin verilen maksimum boyutu aşıyor. Maksimum boyut %d bytes';
$lang_global['error_upload_not_image'] = 'Dosya bilinen bir görüntü formatı değildir';
$lang_global['error_upload_image_failed'] = 'Görüntü İçeriği başarısız';
$lang_global['error_upload_image_width'] = 'The image is not allowed because the width is greater than the maximum of %d pixels';
$lang_global['error_upload_image_height'] = 'The image is not allowed because the height is greater than the maximum of %d pixels';
$lang_global['error_upload_forbidden'] = 'Yükleme yasak';
$lang_global['error_upload_writable'] = '%s dizini yazılabilir değil';
$lang_global['error_upload_urlfile'] = 'URL geçerli değil ve yüklenemiyor';
$lang_global['error_upload_url_notfound'] = 'url bulunamadı';

?>