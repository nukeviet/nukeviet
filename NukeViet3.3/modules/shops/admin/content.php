...
if ($nv_Request->get_int('save', 'post') == 1)
{
    ...
    if (!empty($rowcontent['group_id']))
        $rowcontent['group_id'] = $rowcontent['group_id'] . ',';
    ...
    if (intval($rowcontent['source_id']) > 0)
        $rowcontent['sourcetext'] = "";
    ...
    $rowcontent['product_code'] = change_alias($nv_Request->get_string('product_code', 'post', 0));
    $rowcontent['product_number'] = $nv_Request->get_int('product_number', 'post', 0);
    $rowcontent['product_price'] = $nv_Request->get_string('product_price', 'post', 0);
    $rowcontent['product_price'] = preg_replace('/([^0-9\.])/', '', $rowcontent['product_price']);
    ...
    elseif ($rowcontent['product_price'] <= 0 or !preg_match("/^([0-9]+)(\.[0-9]+)?$/", $rowcontent['product_price']))
    {
...
{
    $rowcontent = $db->sql_fetch_assoc($db->sql_query("SELECT * FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` where `id`=" . $rowcontent['id'] . ""));
    ...
    $rowcontent['note'] = $rowcontent[NV_LANG_DATA . '_note'];
...
if (!empty($rowcontent['bodytext']))
    $rowcontent['bodytext'] = nv_htmlspecialchars($rowcontent['bodytext']);
...
$temp = explode(".", $rowcontent['product_price']);
if (sizeof($temp) == 2)
{
    $rowcontent['product_price'] = number_format($rowcontent['product_price'], strlen($temp[1]), '.', ' ');
}
else
{
    $rowcontent['product_price'] = number_format($rowcontent['product_price'], 0, '.', ' ');
}
