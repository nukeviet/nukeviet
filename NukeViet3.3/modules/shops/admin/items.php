...
$array_search = array("-" => "---", "title" => $lang_module['search_title'], "bodytext" => $lang_module['search_bodytext'], "author" => $lang_module['search_author'], "admin_id" => $lang_module['search_admin'], "product_code" => $lang_module['content_product_code']);
$array_in_rows = array(NV_LANG_DATA . "_title", NV_LANG_DATA . "_bodytext", "product_code");
$array_in_ordername = array(NV_LANG_DATA . "_title", "publtime", "price");
...
while (list($catid_i, $parentid_i, $title_i, $alias_i, $viewcat_i, $subcatid_i, $numlinks_i, $del_cache_time_i, $description_i, $keywords_i, $lev_i) = $db->sql_fetchrow($result))
{
    ...
    $global_array_cat[$catid_i] = array("catid" => $catid_i, "parentid" => $parentid_i, "title" => $title_i, "alias" => $alias_i, "link" => $link_i, "viewcat" => $viewcat_i, "subcatid" => $subcatid_i, "numlinks" => $numlinks_i, "description" => $description_i, "keywords" => $keywords_i);
    $xtitle_i = "";
    if ($lev_i > 0)
    {
        $xtitle_i .= "&nbsp;&nbsp;&nbsp;|";
...
$base_url_price = "" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op . "&per_page=" . $per_page . "&catid=" . $catid . "&stype=" . $stype . "&q=" . $q . "&checkss=" . $checkss . "&ordername=price&order=" . $order2 . "&page=" . $page;
...
$contents .= "<td>" . $lang_module['content_product_code'] . "</td>\n";
$contents .= "<td>" . $lang_module['status'] . "</td>\n";
$contents .= "<td>" . $lang_module['content_admin'] . "</td>\n";
$contents .= "<td>" . $lang_module['content_product_number1'] . "</td>\n";
$contents .= "<td><a href=\"" . $base_url_price . "\">" . $lang_module['content_product_product_price'] . "</a></td>\n";
$contents .= "<td></td>\n";
$contents .= "</thead>";
...
if ($ordername == 'price')
{
    $ord_sql = "ORDER BY `money_unit` ASC, `product_price` " . $order . "";
}
elseif ($ordername == 'title')
{
    $ord_sql = "ORDER BY `" . NV_LANG_DATA . "_title` " . $order . "";
}
else
{
    $ord_sql = "ORDER BY `" . $ordername . "` " . $order . "";
}
$sql = "SELECT id, listcatid, product_code, user_id, " . NV_LANG_DATA . "_title, " . NV_LANG_DATA . "_alias, status , publtime, exptime, product_number, product_price, money_unit FROM " . $from . " " . $ord_sql . " LIMIT " . $page . "," . $per_page;

$result = $db->sql_query($sql);
while (list($id, $catid_i, $product_code_i, $admin_id, $title, $alias, $status, $publtime, $exptime, $product_number, $product_price, $money_unit) = $db->sql_fetchrow($result))
{
    if ($status == 0)
    {
    ...
    $contents .= "<td align=\"center\">" . $product_code_i . "</td>\n";
    $contents .= "<td align=\"center\">" . $status . "</td>\n";
    $contents .= "<td>" . (isset($array_admin[$admin_id]) ? $array_admin[$admin_id] : "") . "</td>\n";
    $contents .= "<td align=\"right\">" . number_format($product_number, 0, '.', ' ') . "</td>\n";
    $contents .= "<td align=\"right\">" . nv_number_format($product_price) . " " . $money_unit . "</td>\n";
