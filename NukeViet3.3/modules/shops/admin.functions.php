...
function nv_show_group_list($parentid = 0)
{
    ...
    if ($num > 0)
    {
        ...
        while (list($groupid, $parentid, $title, $weight, $viewgroup, $numsubgroup, $inhome, $numlinks) = $db->sql_fetchrow($result))
        {
            ...
            if ($numsubgroup > 0)
                $contents .= "  <span style=\"color:#FF0101;\">(" . $numsubgroup . ")</span>";
            $contents .= "</td>\n";
...
function nv_show_block_cat_list()
{
    ...
    if ($num > 0)
    {
        ...
        while ($row = $db->sql_fetchrow($result))
        {
            ...
            if ($numnews)
            {
                $contents .= "<td><a href=\"" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=block&amp;bid=" . $row['bid'] . "\">" . $row[NV_LANG_DATA . '_title'] . " (" . $numnews . " " . $lang_module['product'] . ")</a>";
            }
            else
            {
...
function nv_show_block_list($bid)
{
    ...
    $global_array_cat[0] = array("catid" => 0, "parentid" => 0, "title" => "Other", "alias" => "Other", "link" => $link_i, "viewcat" => "viewcat_page_list", "subcatid" => 0, "numlinks" => 3, "description" => "", "keywords" => "");
