...
function nv_mkdir( $path, $dir_name )
{
	...
	if( ! preg_match( "/^[a-zA-Z0-9-_.]+$/", $dir_name ) )
		return array(
			0,
			sprintf( $lang_global['error_create_directories_name_invalid'], $dir_name )
		);
	$path = @realpath( $path );
	...
	if( ! is_dir( $path ) )
		return array(
			0,
			sprintf( $lang_global['error_directory_does_not_exist'], $path )
		);
	...
	if( ! is_dir( $path . $dir_name ) )
	{
		...
		if( ! is_writable( $path ) )
			return array(
				0,
				sprintf( $lang_global['error_directory_can_not_write'], $path )
			);
	...
	if( ! $res )
		return array(
			0,
			sprintf( $lang_global['error_create_directories_failed'], $dir_name )
		);
...
function nv_deletefile( $file, $delsub = false )
{
	...
	if( empty( $realpath ) )
		return array(
			0,
			sprintf( $lang_global['error_non_existent_file'], $file )
		);
	...
	if( empty( $preg_match ) )
		return array(
			0,
			sprintf( $lang_global['error_delete_forbidden'], $file )
		);
	...
	elseif( is_dir( $realpath ) )// Khong dung FTP
	{
		...
		if( sizeof( $files2 ) and ! $delsub )
		{
			return array(
				0,
				sprintf( $lang_global['error_delete_subdirectories_not_empty'], $path[2] )
			);
		}
		else
		{
			...
			if( sizeof( $files ) )
			{
				foreach( $files as $f )
				{
					...
					if( empty( $unlink[0] ) )
					{
						...
						return array(
							0,
							sprintf( $lang_global['error_delete_failed'], $filename )
						);
					}
				}
			}
			if( ! @rmdir( $realpath ) )
				return array(
					0,
					sprintf( $lang_global['error_delete_subdirectories_failed'], $path[2] )
				);
			else
				return array(
					1,
					sprintf( $lang_global['directory_deleted'], $path[2] )
				);
		}
	}
	else
	...
	else
	{
		return array(
			1,
			sprintf( $lang_global['file_deleted'], $filename )
		);
	}
}
...
function nv_renamefile( $file, $newname )
{
	...
	if( empty( $preg_match ) )
		return array(
			0,
			sprintf( $lang_global['error_rename_forbidden'], $file )
		);
	...
	if( file_exists( $pathinfo['dirname'] . '/' . $newname ) )
		return array(
			0,
			sprintf( $lang_global['error_rename_file_exists'], $newname )
		);
	if( is_dir( $realpath ) and ! preg_match( '/^[a-zA-Z0-9-_]+$/', $newname ) )
		return array(
			0,
			sprintf( $lang_global['error_rename_directories_invalid'], $newname )
		);
	if( ! is_dir( $realpath ) and ! preg_match( '/^[a-zA-Z0-9-_.]+$/', $newname ) )
		return array(
			0,
			sprintf( $lang_global['error_rename_file_invalid'], $newname )
		);
	if( ! is_dir( $realpath ) and $pathinfo['extension'] != nv_getextension( $newname ) )
		return array(
			0,
			sprintf( $lang_global['error_rename_extension_changed'], $newname, $pathinfo['basename'] )
		);
	if( ! @rename( $realpath, $pathinfo['dirname'] . '/' . $newname ) )
	{
		if( ! @nv_copyfile( $realpath, $pathinfo['dirname'] . '/' . $newname ) )
		{
			return array(
				0,
				sprintf( $lang_global['error_rename_failed'], $pathinfo['basename'], $newname )
			);
		}
		else
		{
	...
	return array(
		1,
		sprintf( $lang_global['file_has_been_renamed'], $pathinfo['basename'], $newname )
	);
}
