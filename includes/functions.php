...
function nv_sendmail($from, $to, $subject, $message, $files = '', $AddEmbeddedImage = false, $testmode = false, $cc = [], $bcc = [])
{
    ...
    try {
        ...
        if ($mailer_mode == 'smtp') {
            ...
            $mail->SMTPOptions = ['ssl' => ['verify_peer' => boolval($global_config['verify_peer_ssl']), 'verify_peer_name' => boolval($global_config['verify_peer_name_ssl']), 'allow_self_signed' => true]];
        ...
        if (!$mail->Send()) {
            if (!$testmode and !empty($global_config['notify_email_error'])) {
                nv_insert_notification('settings', 'sendmail_failure', [$sm_parameters['subject'], implode(', ', $sm_parameters['to'])], 0, 0, 0, 1, 2);
            }
