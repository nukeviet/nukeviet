...
class Request
{
	...
	public function __construct( $config, $ip )
	{
		...
		{
			if( ! is_array( $config['allow_request_mods'] ) ) $config['allow_request_mods'] = array( $config['allow_request_mods'] );
	...
	private function Initialize( $my_domains )
	{
		...
		{
			if( preg_match( "/^(" . preg_quote( $this->server_name ) . ")/", $dm ) )
			{
				$valid = true;
				break;
			}
		...
		if( empty( $valid ) )
		{
			header( "Location: " . $this->server_protocol . "://" . $domains[0] . $this->server_port . $this->request_uri );
			exit();
		}
		...
		foreach( $mode as $arr )
		{
			...
			elseif( $arr == 'session' )
			{
				...
				foreach( $names as $name )
				{
					...
					if( ! isset( $_SESSION[$name2] ) ) continue;
