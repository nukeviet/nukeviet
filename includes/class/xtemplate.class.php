...
class XTemplate
{
	 ...
	 * @var array
	 */
	public $allowed_callbacks = array( // Simple string modifiers
		'strtoupper', 'strtolower', 'ucwords', 'ucfirst', 'strrev', 'str_word_count', 'strlen', // String replacement modifiers
		'str_replace', 'str_ireplace', 'preg_replace', 'strip_tags', 'stripcslashes', 'stripslashes', 'substr', 'str_pad', 'str_repeat', 'strtr', 'trim', 'ltrim', 'rtrim', 'nl2br', 'wordwrap', 'printf', 'sprintf', 'addslashes', 'addcslashes', // Encoding / decoding modifiers
		'htmlentities', 'html_entity_decode', 'htmlspecialchars', 'htmlspecialchars_decode', 'urlencode', 'urldecode', // Date / time modifiers
		'date', 'idate', 'strtotime', 'strftime', 'getdate', 'gettimeofday', // Number modifiers
		'number_format', 'money_format', // Miscellaneous modifiers
		'var_dump', 'print_r' );
	 ...
	 * e.g. $xtpl = new XTemplate('file1.tpl');
	 * $xtpl->parse('main');
	 * $xtpl->out('main');
	 * $xtpl->restart('file2.tpl');
	 * $xtpl->parse('main');
	 * $xtpl->out('main');
		...
		foreach ( $con2 as $k => $v )
		{
			...
			if ( preg_match_all( $this->preg_delimiter . "$patt" . $this->preg_delimiter . 'ims', $v, $res, PREG_SET_ORDER ) )
			{
				...
				if ( strtoupper( $block_word ) == $this->block_start_word )
				{
					...
					if ( strtoupper( $block_word ) == $this->block_end_word )
					{
	...
	protected function _getfile( $file )
	{
		...
		if ( ! empty( $this->tpldir ) )
		{
			 ...
			 * 			$xtpl = new XTemplate('myfile.tpl', array('.','/mypath', '/mypath2'));
			 * @since 29/05/2007
			 */
			if ( is_array( $this->tpldir ) )
			{
