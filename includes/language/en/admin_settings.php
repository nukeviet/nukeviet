...
$lang_module['headers'] = 'Config HTTP headers';
...
$lang_module['pp_desc3'] = 'Permissions-Policy is renamed from Feature-Policy (FP). The configurations here will also apply to the Feature-Policy header. FP does not support wildcards, so wildcard configuration lines will be ignored in the FP header';
$lang_module['pp_act'] = 'Active PP';
$lang_module['pp_note'] = 'In the form below, enter each value in one line. Possible values are <code>*</code>, <code>self</code>, <code>()</code>, full domain name https://example.com or wildcard form https://*.example.com. If there is a value of * then all content will be replaced with * corresponding to allowing all at that directive, next if there is () then all content will be replaced with () corresponding to deny access to that directive. Leaving blank means that the directive does not appear in the HTTP header, in which case the browser will decide the default value';
