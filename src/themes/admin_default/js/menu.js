...
$(function() {
    ...
    $('#menu-block .delete-menu-block').on('click', function() {
        if (confirm(nv_is_del_confirm[0])) {
            $.post(script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=blocks&nocache=' + new Date().getTime(), 'del=1&id=' + $(this).data('id') + '&checkss=' + $('#menu-block').data('checkss'), function() {
                location.reload()
            });
        }
    });
    ...
    $('#menulist .item_delete').on('click', function() {
        ...
        if (conf) {
            $.post(script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&nocache=' + new Date().getTime(), 'action=delete&id=' + id + '&parentid=' + parentid + '&mid=' + mid + '&checkss=' + $('#menulist').data('checkss'), function() {
                location.reload()
            });
        }
    });
    ...
    $('#menulist .multi-delete').on('click', function() {
        ...
        if (confirm(nv_is_del_confirm[0])) {
            $.ajax({
                ...
                url: $('#menulist').attr('action'),
                cache: !1,
                data: 'action=delete&mid=' + mid + '&parentid=' + parentid + '&idcheck=' + list + '&checkss=' + $('#menulist').data('checkss')
            }).done(function(a) {
                location.reload()
            })
        }
    ...
    $('#menulist .menu_reload').on('click', function() {
        if (confirm($('#menulist').data('reload-confirm'))) {
            $.post($('#menulist').attr('action'), 'reload=1&mid=' + $('#menulist').data('mid') + '&id=' + $(this).parents('.item').data('id') + '&checkss=' + $('#menulist').data('checkss'), function(res) {
                location.reload()
            });
        }
    });
    ...
    $('#edit').on('change', '[name=module_name]', function() {
        ...
        if (val != '') {
            $.post(script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&nocache=' + new Date().getTime(), 'action=link_module&module=' + val + '&checkss=' + $('#menulist').data('checkss'), function(res) {
                if (res != '') {
                    mod_field.after(res);
    ...
    $('#edit').on('change', '[name=item_menu]', function() {
        $.post(script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&nocache=' + new Date().getTime(), 'action=link_menu&mid=' + $(this).val() + '&parentid=' + $(this).data('parentid') + '&checkss=' + $('#menulist').data('checkss'), function(res) {
