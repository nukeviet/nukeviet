    ...
    <div class="col-sm-12">
        <table class="table table-striped table-bordered tags" data-delete-confirm="{LANG.delete_tag_confirm}">
            <tbody>
                <!-- BEGIN: tag -->
                <tr>
                    <td><input type="text" class="form-control" value="{TAG.name}" data-toggle="auto_update_tag" data-default="{TAG.name}" data-url="{EDIT_LINK}" data-alias="{TAG.alias}" data-checkss="{CHECKSS}"/></td>
                    <td class="text-nowrap" style="width:1%">
                        <a class="btn btn-primary btn-sm" href="{TAG_LINK}&amp;tag={TAG.alias}">{LANG.followers_list}</a>
                        <a class="btn btn-danger btn-sm" href="{FORM_ACTION}" title="{LANG.click_to_delete}" data-toggle="delete_tag" data-tag-alias="{TAG.alias}" data-checkss="{CHECKSS}">{GLANG.delete}</a>
                    </td>
                </tr>
                <!-- END: tag -->
            </tbody>
