<form action="{$FORM_ACTION}" method="post" class="ajax-submit" novalidate data-prevalidate="on">
    <div class="row g-3">
        <div class="col-12">
            <label for="departmentid" class="form-label">{$LANG.department_parent}</label>
        ...
        <div class="col-12">
            <label for="image" class="form-label">{$LANG.supporter_avatar}</label>
            ...
            <div class="help-block text-muted small mt-1" style="display: none;">{$LANG.supporter_avatar_note}</div>
        </div>

        <div class="col-12">
            <label for="phone" class="form-label">{$GLANG.phonenumber} <span class="text-danger">(*)</span></label>
            <div class="input-group">
            ...
            <div class="help-block text-muted small mt-1" style="display: none;">{$GLANG.phone_note_content}</div>
        </div>

        <div class="col-12">
            <label for="email" class="form-label">{$GLANG.email}</label>
            <input type="email" class="form-control" name="email" id="email" value="{$SUPPORTER.email}" maxlength="100" autocomplete="email" />
        </div>

        <div class="col-12">
            <label class="form-label">{$LANG.otherContacts}</label>
            <div class="strs">
                {foreach from=$SUPPORTER.others key=name item=value}
                <div class="str d-flex mb-2">
                    <div class="row flex-grow-1 g-2">
                        <div class="col-5">
                            <input type="text" class="form-control" name="other_name[]" value="{$name}" placeholder="{$LANG.otherVar}" maxlength="250" autocomplete="off" />
                        </div>
                        <div class="col-7">
                            <input type="text" class="form-control" name="other_value[]" value="{$value}" placeholder="{$LANG.otherVal}" maxlength="250" autocomplete="off" />
                        </div>
                    </div>
                    <div class="ms-2">
                        <button class="btn btn-secondary str_add" type="button" title="{$GLANG.add}">
                            <i class="fa-solid fa-plus"></i>
                        </button>
                        <button class="btn btn-secondary str_del" type="button" title="{$GLANG.delete}">
