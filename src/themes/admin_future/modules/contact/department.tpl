<div class="card mb-3">
    <div class="card-body">
        <div class="table-responsive-lg table-card">
            <table class="table table-striped list mb-0" data-checkss="{$CHECKSS}" data-url="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&amp;{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$smarty.const.NV_OP_VARIABLE}={$OP}">
                <thead>
                    <tr>
                        <th class="text-center text-nowrap" style="width:1%;">{$LANG->getModule('number')}</th>
                        <th class="text-nowrap">{$LANG->getModule('part_row_title')}</th>
