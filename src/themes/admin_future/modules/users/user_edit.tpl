...
<form class="ajax-submit" method="post" novalidate
      action="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&amp;{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$smarty.const.NV_OP_VARIABLE}={$OP}&amp;userid={$DATA.userid}">
    <div class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">{$LANG->getModule('edit_title')}</h5>
        </div>
        <div class="card-body">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8 col-lg-6 col-xxl-5">
                    <div class="input-group" style="max-width: 180px;">
                        <input type="text" class="form-control datepicker" id="birthday_field" name="birthday" value="{$sf.value}" maxlength="10" autocomplete="off">
            ...
            {* Hình đại diện *}
            <div class="row mb-3">
                <div class="col-sm-3 col-form-label text-sm-end">{$LANG->getModule('avatar')}</div>
                <div class="col-sm-8 col-lg-6 col-xxl-5">
                    {if $PHOTO.src}
                    <div id="current-photo" class="mb-2">
                        <img id="imageresource" alt="{$DATA.username}" src="{$PHOTO.src}" width="{$PHOTO.width}" height="{$PHOTO.height}" class="img-thumbnail" style="max-height:120px;cursor:pointer;">
                        <div class="mt-1">
                            <button type="button" class="btn btn-sm btn-outline-danger" id="current-photo-btn" data-toggle="deletephoto">
                                <i class="fa-solid fa-trash"></i> {$LANG->getModule('delete')}
                            </button>
                        </div>
                        <input type="hidden" name="delpic" id="photo_delete" value="0">
                    </div>
                    {/if}
            ...
            <div class="row mb-3">
                <div class="col-sm-8 col-lg-6 col-xxl-5 offset-sm-3">
                    <div class="form-check form-switch mt-1">
                        <input class="form-check-input" type="checkbox" id="view_mail_field" name="view_mail" value="1"{if $DATA.view_mail} checked{/if}>
                        <label class="form-check-label" for="view_mail_field">{$LANG->getModule('show_email')}</label>
                    </div>
                </div>
            </div>

            {* Là người dùng chính thức (chỉ hiện khi là tài khoản mới - group 7) *}
            {if $IS_NEW_USER}
            <div class="row mb-3">
                <div class="col-sm-8 col-lg-6 col-xxl-5 offset-sm-3">
                    <div class="form-check form-switch mt-1">
                        <input class="form-check-input" type="checkbox" id="is_official_field" name="is_official" value="1">
                        <label class="form-check-label" for="is_official_field">{$LANG->getModule('set_official_note')}</label>
                    </div>
                </div>
            </div>
            {/if}
            ...
            {* Danh sách nhóm *}
            {if $GROUP_EXISTS}
            <div class="row mb-3{if $IS_NEW_USER} d-none{/if}" id="ctn-list-groups">
                <div class="col-sm-3 col-form-label text-sm-end">{$LANG->getModule('in_group')}</div>
                <div class="col-sm-8 col-lg-6 col-xxl-5">
                    {foreach from=$GROUPS item=grp}
                    <div class="d-flex align-items-center gap-3 mb-1">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="group_{$grp.id}" name="group[]" value="{$grp.id}"{if $grp.checked} checked{/if}{if $grp.disabled} disabled{/if}>
                            <label class="form-check-label" for="group_{$grp.id}" style="min-width: 180px;">{$grp.title}</label>
                        </div>
                        <div class="form-check group_default"{if !$grp.checked} style="display:none"{/if}>
                            <input class="form-check-input" type="radio" id="group_default_{$grp.id}" name="group_default" value="{$grp.id}"{if $grp.is_default} checked{/if}>
            ...
            <div class="row mb-3">
                <div class="col-sm-8 col-lg-6 col-xxl-5 offset-sm-3">
                    <div class="form-check">
                        ...
                        <label class="form-check-label small" for="adduser_email_field">{$LANG->getModule('adduser_email1_note1')}</label>
                    </div>
                </div>
            </div>
            ...
            {if $SHOW_CHANGEPASS}
            <hr>
            <h6 class="mb-3"><i class="fa-solid fa-key"></i> {$LANG->getModule('edit_password_note')}</h6>

            <div class="row mb-3">
                <label for="password1" class="col-sm-3 col-form-label text-sm-end">
                    {$LANG->getModule('password')}
                </label>
                <div class="col-sm-8 col-lg-6 col-xxl-5">
                    <div class="input-group">
                        <input type="password"
        ...
        <div class="card-footer text-center">
            ...
            <button class="btn btn-primary" type="submit">
                <i class="fa-solid fa-floppy-disk"></i> {$LANG->getModule('edit_title')}
            </button>
        </div>
    </div>
</form>
