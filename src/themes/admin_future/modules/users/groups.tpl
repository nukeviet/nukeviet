            ...
            <table class="table table-striped align-middle table-sticky mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            {if $row.show_weight}
                            <button id="group_weight_{$row.group_id}" data-toggle="changegroupweight" data-mod="weight" data-min="{$START_WEIGHT}" data-num="{$MAX_WEIGHT}" data-id="{$row.group_id}" data-current="{$row.weight}" data-checkss="{$CHECKSS}" data-msgerror="{$LANG->getModule('errorChangeWeight')}" type="button" class="btn btn-secondary btn-sm d-flex align-items-center gap-1 justify-content-between btn-dropdown-tool fw-75">
                        ...
                        <td class="text-center"><input data-id="{$row.group_id}" data-checkss="{$CHECKSS}" type="checkbox" class="actGroup form-check-input" name="act_{$row.group_id}" value="1"{if $row.act} checked{/if}{if $row.disabled} disabled{/if}></td>
                        <td class="text-center">
                            ...
                            {if $row.can_delete}<a class="delGroup btn btn-danger btn-sm" href="#" data-id="{$row.group_id}" data-checkss="{$CHECKSS}" aria-label="{$LANG->getGlobal('delete')}"><i class="fa-solid fa-trash" data-icon="fa-trash"></i> {$LANG->getGlobal('delete')}</a>{/if}
                            {/if}
                        </td>
                    </tr>
                    {/foreach}
...
{if $SHOW_ACTION_JS}
<div class="mt-3">
    <a class="btn btn-danger" href="#" data-toggle="delInactiveGroup" data-checkss="{$CHECKSS}" data-msgconfirm="{$LANG->getModule('delConfirm')} ?" aria-label="{$LANG->getModule('group_del_inactive')}"><i class="fa-solid fa-trash"></i> {$LANG->getModule('group_del_inactive')}</a>
</div>
{/if}
