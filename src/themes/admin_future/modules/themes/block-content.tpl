...
<form class="p-1 ajax-submit" method="post" action="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&amp;{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$smarty.const.NV_OP_VARIABLE}={$OP}&amp;selectthemes={$SELECTTHEMES}{if not empty($BLOCKREDIRECT)}&amp;blockredirect={$BLOCKREDIRECT}{/if}" id="block-content-form" data-page-url="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&amp;{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}" data-selectthemes="{$SELECTTHEMES}" data-callback="nvBlockCtCallback" data-checkss-ajax="{$CHECKSS_AJAX}" novalidate>
