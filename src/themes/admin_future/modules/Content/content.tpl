<form id="form-content" method="post" action="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&amp;{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$smarty.const.NV_OP_VARIABLE}={$OP}{if not empty($ID)}&amp;id={$ID}{/if}" novalidate class="ajax-submit">
    <div class="row g-3">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <select class="form-select" name="catid" id="catid">
                        <option value="0">{$LANG->getModule('cat_unclassified')}</option>
                        {foreach from=$CATS item=cat}
                        <option value="{$cat->catid}"{if $cat->catid eq $DATA.catid} selected{/if}>{$cat->title}</option>
                        {/foreach}
                    </select>
                </div>
            </div>
