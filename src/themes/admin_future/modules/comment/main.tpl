                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <input type="checkbox" name="activecheckbox" id="change_active_{$ROW.cid}" onclick="nv_change_active('{$ROW.cid}')" class="form-check-input m-0 align-middle"{if $ROW.status} checked="checked"{/if} data-checkss="{$CHECKSS}">
                        </td>
                        <td class="text-right">
                            {if !empty($ROW.attach)}
                            ...
                            <a class="btn btn-danger btn-sm deleteone mt-1" href="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&amp;{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$smarty.const.NV_OP_VARIABLE}=del&amp;list={$ROW.cid}" title="{$LANG->getModule('delete')}" data-checkss="{$CHECKSS}"><i class="fa-solid fa-trash fa-fw" aria-hidden="true"></i><span class="d-none d-xl-inline">{$LANG->getModule('delete')}</span></a>
                        </td>
                    </tr>
                    {/foreach}
                </tbody>
