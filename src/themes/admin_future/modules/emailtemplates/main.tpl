                    ...
                    <tr>
                        ...
                        <td class="text-nowrap text-center">
                            <div class="hstack gap-1 d-inline-flex">
                                <a href="{$NV_BASE_ADMINURL}index.php?{$NV_LANG_VARIABLE}={$NV_LANG_DATA}&amp;{$NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$NV_OP_VARIABLE}=test&amp;emailid={$row.emailid}" class="btn btn-sm btn-secondary text-center" title="{$LANG->get('test')}" aria-label="{$LANG->get('test')}" data-bs-toggle="tooltip" data-bs-trigger="hover"><i class="fa-solid fa-fw fa-paper-plane"></i></a>
                                <a href="{$NV_BASE_ADMINURL}index.php?{$NV_LANG_VARIABLE}={$NV_LANG_DATA}&amp;{$NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$NV_OP_VARIABLE}=contents&amp;copyid={$row.emailid}" class="btn btn-sm btn-secondary text-center" title="{$LANG->get('copy')}" aria-label="{$LANG->get('copy')}" data-bs-toggle="tooltip" data-bs-trigger="hover"><i class="fa-solid fa-fw fa-copy"></i></a>
                                <a href="{$NV_BASE_ADMINURL}index.php?{$NV_LANG_VARIABLE}={$NV_LANG_DATA}&amp;{$NV_NAME_VARIABLE}={$MODULE_NAME}&amp;{$NV_OP_VARIABLE}=contents&amp;emailid={$row.emailid}" class="btn btn-sm btn-secondary text-center" title="{$LANG->get('edit')}" aria-label="{$LANG->get('edit')}" data-bs-toggle="tooltip" data-bs-trigger="hover"><i class="fa-solid fa-fw fa-pencil"></i></a>
                                {if not $row.is_system and not $row.module_name}<a href="#" class="btn btn-sm btn-secondary text-center link-danger" data-click="deltpl" data-emailid="{$row.emailid}" data-checkss="{$CHECKSS}" title="{$LANG->get('delete')}" aria-label="{$LANG->get('delete')}" data-bs-toggle="tooltip" data-bs-trigger="hover"><i class="fa-solid fa-fw fa-trash" data-icon="fa-trash"></i></a>{/if}
                            </div>
                        </td>
                    </tr>
                    {/foreach}
