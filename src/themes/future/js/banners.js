...
function loadStat() {
    ...
    if (!!type && !!month && !!ads) {
        $('#chartdata').html('<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div>').show();
        var width = Math.floor($('#chartdata').width()) > 500 ? 700 : 500;
        charturl += '&type=' + type + '&month=' + month + '&ads=' + ads + '&width=' + width;
        var img = new Image();
        $(img).on('load', function() {
            $('#chartdata').html('<img src="' + img.src + '" class="img-fluid"/>');
        });
        img.src = charturl;
    } else {
        $('#chartdata').hide();
