            ...
            if (isset($site_mods[$modname]) and !empty($site_mods[$modname]['funcs'])) {
                ...
                if (!empty($modvalues['funcs'])) {
                    if ($modvalues['module_file'] == 'news' or $modvalues['module_file'] == 'weblinks') {
                        $where_cat = $modvalues['module_file'] == 'news' ? 'status=1' : 'inhome=1';
                        $sql = 'SELECT title, alias FROM ' . NV_PREFIXLANG . '_' . $modvalues['module_data'] . '_cat WHERE parentid=0 AND ' . $where_cat . ' ORDER BY weight ASC LIMIT 10';
