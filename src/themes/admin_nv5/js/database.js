...
function nv_delete_sampledata(sname) {
    if (confirm(nv_is_del_confirm[0])) {
        $.post(script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=sampledata&nocache=' + new Date().getTime(), 'delete=1&sname=' + encodeURIComponent(sname), function(res) {
    ...
    $.post(url + '&nocache=' + new Date().getTime(), data, function(res) {
        ...
        if (res.next) {
            ...
            if (!res.finish) {
                ...
                $('[type="submit"] .lo', $this).addClass('d-none');
...
$(function() {
    $('#sampledataarea form').submit(function(e) {
        ...
        $('[type="submit"] .lo', $this).removeClass('d-none');
        $('<p>' + $('#sampledataarea').data('init') + '</p>').insertAfter('#spdresulttop');
        $('#spdresult').show();
