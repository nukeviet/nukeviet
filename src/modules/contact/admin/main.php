...
if (!empty($contact_allowed['reply'])) {
    ...
    if ($nv_Request->isset_request('reply', 'post') or $nv_Request->isset_request('forward', 'post')) {
        ...
        $stmt = $db->prepare('SELECT * FROM ' . NV_MOD_TABLE . '_send WHERE id = :id AND ' . $db_deps);
        ...
        $stmt->bindValue(':reply_content', $mess_content, PDO::PARAM_STR);
        $stmt->bindValue(':reply_aid', $admin_info['admin_id'], PDO::PARAM_INT);
        $stmt->execute();

        $stmt = $db->prepare('UPDATE ' . NV_MOD_TABLE . '_send SET is_reply = :is_reply WHERE id = :id');
        $stmt->bindValue(':is_reply', $mode, PDO::PARAM_INT);
        $stmt->bindValue(':id', $id, PDO::PARAM_INT);
        $stmt->execute();
...
if (!empty($contact_allowed['exec'])) {
    ...
    if ($nv_Request->isset_request('mark', 'post')) {
        ...
        if (in_array($mark, ['read', 'unread', 'processed', 'unprocess'], true)) {
            ...
            $in_sends = implode(',', array_map('intval', $sends));
            if (($mark == 'read' or $mark == 'unread')) {
                if ($mk) {
                    $stmt = $db->prepare('UPDATE ' . NV_MOD_TABLE . '_send SET is_read = 1 WHERE id IN (' . $in_sends . ') AND ' . $db_deps);
                } else {
                    $stmt = $db->prepare('UPDATE ' . NV_MOD_TABLE . '_send SET is_read = 0, is_processed = 0, processed_by = 0, processed_time = 0 WHERE id IN (' . $in_sends . ') AND ' . $db_deps);
                }
                $stmt->execute();
            } else {
                $stmt = $db->prepare('UPDATE ' . NV_MOD_TABLE . '_send SET is_read = 1, is_processed = :is_processed, processed_by = :processed_by, processed_time = :processed_time WHERE id IN (' . $in_sends . ') AND ' . $db_deps);
                $stmt->bindValue(':is_processed', $mk, PDO::PARAM_INT);
                $stmt->bindValue(':processed_by', $mk ? (int) $admin_info['userid'] : 0, PDO::PARAM_INT);
                $stmt->bindValue(':processed_time', $mk ? NV_CURRENTTIME : 0, PDO::PARAM_INT);
                $stmt->execute();
                nv_insert_logs(NV_LANG_DATA, $module_name, ($mk ? $nv_Lang->getModule('mark_as_processed') : $nv_Lang->getModule('mark_as_unprocess')), 'ID: ' . $in_sends, $admin_info['userid']);
            }
            nv_jsonOutput([
                'status' => 'ok',
                'mess' => ''
    ...
    if ($nv_Request->isset_request('delete', 'post')) {
        ...
        if ($type == 3) {
            if (defined('NV_IS_SPADMIN')) {
                ...
                if (!empty($ids)) {
                    ...
                    $db->exec('TRUNCATE TABLE ' . NV_MOD_TABLE . '_send');
                    $db->exec('TRUNCATE TABLE ' . NV_MOD_TABLE . '_reply');
                }
            }
            /*else {
                ...
                if (!empty($ids)) {
                    nv_delete_notification(NV_LANG_DATA, $module_name, 'contact_new', $ids);
                    $stmt = $db->prepare('DELETE FROM ' . NV_MOD_TABLE . '_send WHERE id IN (' . $ids . ')');
                    $stmt->execute();
                    $stmt = $db->prepare('DELETE FROM ' . NV_MOD_TABLE . '_reply WHERE id IN (' . $ids . ')');
                    $stmt->execute();
                }
            }*/
        }
        ...
        // Xóa đơn lẻ
            ...
            $stmt = $db->prepare('SELECT COUNT(*) FROM ' . NV_MOD_TABLE . '_send WHERE id = :id AND ' . $db_deps);
            $stmt->bindValue(':id', $id, PDO::PARAM_INT);
            $stmt->execute();
            ...
            if ($exist) {
                ...
                $stmt = $db->prepare('DELETE FROM ' . NV_MOD_TABLE . '_send WHERE id = :id');
                $stmt->bindValue(':id', $id, PDO::PARAM_INT);
                $stmt->execute();

                $stmt = $db->prepare('DELETE FROM ' . NV_MOD_TABLE . '_reply WHERE id = :id');
                $stmt->bindValue(':id', $id, PDO::PARAM_INT);
                $stmt->execute();
            } else {
                nv_jsonOutput([
...
if (!empty($contact_allowed['view'])) {
    ...
    if ($nv_Request->isset_request('id', 'get')) {
        ...
        if (empty($row['is_read'])) {
            ...
            $stmt = $db->prepare('UPDATE ' . NV_MOD_TABLE . '_send SET is_read = 1 WHERE id = :id');
            $stmt->bindValue(':id', $id, PDO::PARAM_INT);
            $stmt->execute();
            $row['is_read'] = 1;
        }
        ...
        if (empty($row['read_admins']) or !isset($row['read_admins'][$admin_info['admin_id']])) {
            ...
            $stmt = $db->prepare('UPDATE ' . NV_MOD_TABLE . '_send SET read_admins = :read_admins WHERE id = :id');
            $stmt->bindValue(':read_admins', $read_admins, PDO::PARAM_STR);
            $stmt->bindValue(':id', $id, PDO::PARAM_INT);
            $stmt->execute();
        }
        ...
        if (defined('NV_IS_SPADMIN') or !empty($row['auto_forward']) or !empty($row['processed_by'])) {
            ...
            $admins_in = implode(',', array_map('intval', $admins));
            $stmt = $db->prepare('SELECT t1.admin_id, t2.username as admin_login, t2.first_name, t2.last_name FROM ' . NV_AUTHORS_GLOBALTABLE . ' t1 INNER JOIN ' . NV_USERS_GLOBALTABLE . ' t2 ON t1.admin_id = t2.userid WHERE t1.admin_id IN (' . $admins_in . ')');
            $stmt->execute();
        ...
        if (!empty($row['sender_id'])) {
            ...
            $stmt->bindValue(':userid', $row['sender_id'], PDO::PARAM_INT);
            $stmt->execute();
            $userinfo = $stmt->fetch();
            $stmt->closeCursor();
        ...
        if (!empty($row['is_reply'])) {
            $stmt = $db->prepare('SELECT * FROM ' . NV_MOD_TABLE . '_reply WHERE id = :id ORDER BY reply_time DESC');
...
if (!empty($contact_allowed['view'])) {
    ...
    $sql = 'SELECT COUNT(*) FROM ' . NV_MOD_TABLE . '_send WHERE cid IN (' . $in . ')';
