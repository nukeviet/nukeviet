        ...
        while ($_row = $_result->fetch()) {
            ...
            if (!isset($_row['language'][$lang])) {
                if (!empty($_row['is_system'])) {
                    $_row['language'][$lang] = [0 => $nv_Lang->getModule($_row['field']), 1 => ''];
                } elseif (isset($_row['language'][$set_lang_data])) {
                    $_row['language'][$lang] = [0 => ucfirst(nv_EncString($_row['language'][$set_lang_data][0])), 1 => ucfirst(nv_EncString($_row['language'][$set_lang_data][1]))];
                } else {
                    $_copy_lang = current($_row['language']);
                    $_row['language'][$lang] = [0 => ucfirst(nv_EncString($_copy_lang[0])), 1 => ucfirst(nv_EncString($_copy_lang[1]))];
                }
