...
if ($id > 0 and in_array($point, $array_point, true) and $checkss == md5($id . NV_CHECK_SESSION)) {
    if (!empty($time_set)) {
        exit($nv_Lang->getModule('rating_error2') . '|0|0');
    }
    ...
    if (isset($row['allowed_rating']) and $row['allowed_rating'] == 1) {
        ...
        $contents = sprintf($nv_Lang->getModule('stringrating'), $row['total_rating'] + $point, $row['click_rating'] + 1);
        exit($contents . '|' . round(($row['total_rating'] + $point) / ($row['click_rating'] + 1), 1) . '|' . ($row['click_rating'] + 1));
    }
}

exit($nv_Lang->getModule('rating_error1') . '|0|0');
