...
if ($catid > 0) {
    if ((defined('NV_IS_ADMIN_MODULE') or ($parentid > 0 and isset($array_cat_admin[$admin_id][$parentid]) and $array_cat_admin[$admin_id][$parentid]['admin'] == 1))) {
        ...
        if ((int) $check_parentid > 0) {
            $contents = 'ERR_CAT_' . sprintf($nv_Lang->getModule('delcat_msg_cat'), $check_parentid);
        } else {
            ...
            if ((int) $check_rows > 0) {
                if ($delallcheckss == md5($catid . NV_CHECK_SESSION)) {
                    ...
                    if (empty($delcatandrows) and empty($movecat)) {
                        ...
                        $xtpl->assign('TITLE', sprintf($nv_Lang->getModule('delcat_msg_rows_select'), $title, $check_rows));
                        ...
                        if ($catidnews > 0) {
                            ...
                            while ($row = $sql->fetch()) {
                                ...
                                if (!in_array($catidnews, array_map('intval', $arr_catid_news), true)) {
                                    try {
                    ...
                    $contents = 'ERR_ROWS_' . $catid . '_' . md5($catid . NV_CHECK_SESSION) . '_' . sprintf($nv_Lang->getModule('delcat_msg_rows'), $check_rows);
                }
            }
        }
