    ...
    if ($nv_Request->isset_request('submit', 'post') and $userid > 0) {
        ...
        if ($admin_module == 1 or $admin_module == 2) {
            ...
            while ($row = $result_cat->fetch()) {
                ...
                $admin_i = (in_array($row['catid'], $array_admin, true)) ? 1 : 0;
                if ($admin_i) {
                    ...
                    $add_content_i = (in_array($row['catid'], $array_add_content, true)) ? 1 : 0;
                    $pub_content_i = (in_array($row['catid'], $array_pub_content, true)) ? 1 : 0;
                    $edit_content_i = (in_array($row['catid'], $array_edit_content, true)) ? 1 : 0;
                    $del_content_i = (in_array($row['catid'], $array_del_content, true)) ? 1 : 0;
                    $app_content_i = (in_array($row['catid'], $array_app_content, true)) ? 1 : 0;
                    if (!empty($row['subcatid'])) {
                        $array_subcatid_i = explode(',', $row['subcatid']);
                        foreach ($array_subcatid_i as $value) {
