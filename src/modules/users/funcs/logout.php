...
if (defined('NV_IS_USER_FORUM') or defined('SSO_SERVER')) {
    ...
    if ($user_info['current_mode'] == 4 and preg_match('/^[a-z0-9_]+$/i', $user_info['openid_server']) and module_file_exists('users/login/cas-' . $user_info['openid_server'] . '.php')) {
...
$url_redirect = htmlspecialchars(nv_url_rewrite($url_redirect, true), ENT_QUOTES, 'UTF-8');
