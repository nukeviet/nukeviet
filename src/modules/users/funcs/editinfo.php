...
if ((int) $row['safemode'] > 0) {
    ...
    if ($checkss == $array_data['checkss'] and $type == 'safe_deactivate') {
        ...
        $safe_key = nv_substr($nv_Request->get_title('safe_key', 'post', ''), 0, 32);
...
if ($checkss == $array_data['checkss'] and $array_data['type'] == 'basic') {
    $array_data['first_name'] = isset($array_field_config['first_name']) ? nv_substr($nv_Request->get_title('first_name', 'post', ''), 0, 255) : $row['first_name'];
    $array_data['last_name'] = isset($array_field_config['last_name']) ? nv_substr($nv_Request->get_title('last_name', 'post', ''), 0, 255) : $row['last_name'];
    $array_data['gender'] = isset($array_field_config['gender']) ? nv_substr($nv_Request->get_title('gender', 'post', ''), 0, 1) : $row['gender'];
    $array_data['birthday'] = isset($array_field_config['birthday']) ? nv_substr($nv_Request->get_title('birthday', 'post', ''), 0, 10) : $row['birthday'];
    ...
    // Username
    $nv_username = nv_substr($nv_Request->get_title('username', 'post', ''), 0, $global_config['nv_unickmax']);
    ...
    // Email
    $nv_email = nv_strtolower(nv_substr($nv_Request->get_title('email', 'post', ''), 0, 100));
    $nv_password = $nv_Request->get_title('password', 'post', '');
    ...
    // Question
    $array_data['question'] = isset($array_field_config['question']) ? nv_substr($nv_Request->get_title('question', 'post', ''), 0, 255) : $row['question'];
    $array_data['answer'] = isset($array_field_config['answer']) ? nv_substr($nv_Request->get_title('answer', 'post', ''), 0, 255) : $row['answer'];
    $nv_password = $nv_Request->get_title('nv_password', 'post', '');
