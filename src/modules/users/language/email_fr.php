...
$module_emails[Emails::DELETE_ACCOUNT_COMPLETED] = [
    ...
    'c' => '<p>{$greeting_user}</p> <p>Comme demandé, votre compte a été <strong>supprimé définitivement</strong> de notre système. Cette action ne peut pas être annulée.</p> <p>Toutes les données personnelles, l\'historique des activités et les services liés à ce compte ont été supprimés de manière irrécupérable.</p> <p>{$deadline}.</p> <p>Nous regrettons de devoir vous dire au revoir et espérons avoir l\'occasion de vous servir à nouveau dans le futur.</p> <p style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; font-size: 14px; color: #6b7280; font-style: italic; margin-bottom: 0px;"> Il s\'agit d\'une notification obligatoire concernant l\'activité de votre compte. Veuillez ne pas répondre et ignorer cet e-mail si vous ne comprenez pas le contenu. </p>'
];
