...
if (!empty($checkss) and csrf_check($checkss, $action_csrf_key)) {
    ...
    $exp_array = [];
    $sql = 'SELECT id, listcatid, admin_id as post_id, publtime, exptime, status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE id IN (' . implode(',', $id_array) . ')';
    $result = $db->query($sql);
    while ($_row = $result->fetch()) {
        if (($_row['exptime'] == 0 or $_row['exptime'] > NV_CURRENTTIME) and $_row['status'] != 4 and $_row['status'] <= $global_code_defined['row_locked_status']) {
                    ...
                    if (isset($array_cat_admin[$admin_id][$catid_i])) {
                        if ($array_cat_admin[$admin_id][$catid_i]['admin'] == 1) {
                            ...
                            if ($array_cat_admin[$admin_id][$catid_i]['edit_content'] == 1) {
                                ...
                                ++$check_edit;
                            } elseif (($_row['status'] == 0 or $_row['status'] == 6) and $_row['post_id'] == $admin_id) {
                                ++$check_edit;
            ...
            if ($check_permission > 0) {
                $stmt_update = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET status = 5 WHERE id = :id');
                $stmt_update->bindValue(':id', $_row['id'], PDO::PARAM_INT);
                $stmt_update->execute();

                foreach ($arr_catid as $catid_i) {
                    $stmt_update_cat = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_' . intval($catid_i) . ' SET status = 5 WHERE id = :id');
                ...
                Logs::saveLogStatusPost($_row['id'], 5);
            }
        }
    }
    $result->closeCursor();
