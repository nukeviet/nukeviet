...
foreach ($comments as $row) {
    ...
    if (defined('NV_IS_SPADMIN')) {
        ...
        $mod_list = "'" . implode("', '", array_keys($site_mod_comm)) . "'";

        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE cid=' . $row['cid'] . ' AND module IN (' . $mod_list . ')');
        $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET pid=' . $row['pid'] . ' WHERE pid=' . $row['cid'] . ' AND module IN (' . $mod_list . ')');
    }
