...
$lang_module['turnstile_sitekey'] = 'CloudFlare Turnstile Site key';
$lang_module['turnstile_secretkey'] = 'CloudFlare Turnstile Secret key';
...
$lang_module['captcha_turnstile'] = 'CloudFlare Turnstile';
...
$lang_module['captcha_type_turnstile_note'] = 'CloudFlare Turnstile will be replaced with Image-Captcha until you declare the Site key and Secret key of CloudFlare Turnstile.';
...
$lang_module['admin_2step_warning'] = 'Access lock is not always available on all devices, selecting only one access lock method will limit the scope of admin login. You should be aware of this issue';
$lang_module['admin_2step_error'] = 'You are enforcing two-step verification in administration but have not selected any allowed methods. This will prevent you from logging into the administration. Please select at least one method';
...
$lang_module['trusted_proxy_note_strip'] = 'If the website sits behind an internal proxy or a provider other than Cloudflare, configure the proxy to overwrite or strip every client-supplied IP header; otherwise an attacker can still spoof the IP by sending forged headers. The system reads four headers: CF-Connecting-IP, X-Forwarded-For, Forwarded and X-Real-IP. Example for Nginx: proxy_set_header CF-Connecting-IP ""; proxy_set_header Forwarded ""; proxy_set_header X-Real-IP $remote_addr; and proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;';
$lang_module['trusted_proxy_enable'] = 'Enable';
$lang_module['trusted_proxy_enable_des'] = 'Only trust proxy IP headers when this option is enabled. When disabled, the system always uses the direct connecting IP (REMOTE_ADDR) and ignores every client-supplied IP header';
...
$lang_module['trusted_proxy_warn_untrusted'] = 'Warning: trusting proxies is enabled, but the direct connecting IP of the current request (%s) is not in the list below, so the system still ignores the IP headers and records this very IP. Common causes: the Cloudflare ranges in the list are outdated, or the internal reverse proxy ranges are missing. Click "Fetch Cloudflare ranges" to refresh them, or add the missing proxy ranges and save. Only add ranges of a proxy or CDN you control; adding unknown ranges opens the door to IP spoofing.';
$lang_module['trusted_proxy_warn_disabled'] = 'Warning: the current request carries proxy IP headers but trusting proxies is <strong>disabled</strong>, so the system records your IP as %s, that is the proxy IP, not the real one. If the website really sits behind a reverse proxy or CDN, every visitor shares a single IP: a failed-login lockout will affect all users, IP bans lose their selectivity and logs become useless for tracing. Enable the option below and enter the correct proxy ranges.';
