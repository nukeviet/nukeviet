...
function nv_register_block(string $tag, string $name = '', string $module = '')
{
    ...
    if (!preg_match('/^(?!_)[a-zA-Z0-9_]+(?<!_)$/', $tag)) {
        ...
        throw new InvalidArgumentException('nv_register_block: Invalid tag name ' . nv_htmlspecialchars($tag));
    }
    $name = nv_htmlspecialchars($name ?: $tag);
    if (nv_strlen($name) > 100) {
        http_response_code(500);
        throw new InvalidArgumentException('nv_register_block: Block name too long, max 100 chars');
    }
...
function nv_unregister_block(string $tag, string $module = '', bool $all = false)
{
    ...
    if (empty($tag) and !$all) {
        http_response_code(500);
        throw new InvalidArgumentException('nv_unregister_block: Method call is not allowed!');
    }
