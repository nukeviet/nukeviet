...
nv_add_hook($module_name, 'get_global_admin_theme', $priority, function ($vars) {
    ...
    if (($module_info['module_file'] ?? '') == 'users' and in_array($op, ['config', 'fields', 'siteterms', 'question', 'groups', 'user_waiting', 'user_waiting_remail', 'user_add', 'edit_oauth', 'edit_2step'])) {
        return $new_theme;
    }
