...
nv_add_hook($module_name, 'get_module_admin_theme', $priority, function ($vars) {
    ...
    if (($module_info['module_file'] ?? '') == 'news' and in_array($op, ['drafts', 'report', 'content', 'tags', 'main', 'setting', 'authors'])) {
        return $new_theme;
    }
    if (($module_info['module_file'] ?? '') == 'users' and in_array($op, ['config', 'fields', 'siteterms', 'question', 'groups', 'user_waiting', 'user_waiting_remail'])) {
        return $new_theme;
    }
    ...
    if (($module_info['module_file'] ?? '') == 'contact' and in_array($op, ['main', 'department', 'config', 'send', 'supporter'])) {
        return $new_theme;
    }
