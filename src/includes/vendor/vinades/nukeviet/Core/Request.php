...
class Request
{
    ...
    public function __construct($config, $ip)
    {
        ...
        if (isset($config['request_default_mode']) and !empty($config['request_default_mode']) and in_array($config['request_default_mode'], $this->allow_request_mods, true)) {
    ...
    private function filterTags($source)
    {
        ...
        if (in_array('iframe', $this->disabletags, true)) {
            if (preg_match_all("/<iframe[a-z0-9\s\=\"]*src\=\"(http(s)?\:)?\/\/([w]{3})?\.youtube[^\/]+\/embed\/([^\?]+)(\?[^\"]+)?\"[^\>]*\><\/iframe>/isU", $source, $match)) {
                foreach ($match[0] as $key => $_m) {
                    $vid = $match[4][$key];
                    $width = (int) (preg_replace("/^(.*)width\=\"([\d]+)\"(.*)$/isU", '\\2', $_m));
                    $height = (int) (preg_replace("/^(.*)height\=\"([\d]+)\"(.*)$/isU", '\\2', $_m));
    ...
    {
        return abs((int) ($this->get_value($name, $mode, $default, $decode)));
    }
    ...
    private function getAllowOriginHeaderValue()
    {
        ...
        if (!$this->restrictCORSDomains or in_array($origin, $this->validCORSDomains, true)) {
