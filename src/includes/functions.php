...
function nv_is_myreferer($referer = '')
{
    ...
    $referer = preg_replace(['/^[a-zA-Z]+\:\/\/([w]+\.)?/', '/^[w]+\./'], '', $referer);
 ...
 *
 * @param string $mail
 * @param bool   $return @since 4.3.08
 * @return string
 */
...
function nv_user_in_groups($groups_view)
{
    ...
    } elseif (in_array(5, $groups_view, true)) {
        // Guest
        return true;
    }
...
function nv_sendmail($from, $to, $subject, $message, $files = '', $AddEmbeddedImage = false, $cc = '', $bcc = '', $testmode = false, $template = true)
{
    ...
    try {
        ...
        } elseif (!in_array('mail', $sys_info['disable_functions'], true)) {
            $mail->IsMail();
...
function nv_site_mods()
{
    ...
    if (defined('NV_SYSTEM')) {
        ...
        foreach ($site_mods as $m_title => $row) {
            if (defined('NV_IS_SPADMIN')) {
                $allowed = true;
            } elseif (!empty($row['admins']) and in_array((int) $admin_info['admin_id'], array_map('intval', explode(',', $row['admins'])), true)) {
                $allowed = true;
            } else {
                unset($site_mods[$m_title]);
            }
...
function nv_set_authorization()
{
    ...
    return ['auth_user' => $auth_user, 'auth_pw' => $auth_pw];
}
...
function nv_local_api($cmd, $params, $adminidentity = '', $module = '')
{
     ...
     */
    if ($module != '' and NukeViet\Api\Api::getAdminLev() > 2 and !in_array(NukeViet\Api\Api::getAdminId(), explode(',', $sys_mods[$module]['admins']), true)) {
