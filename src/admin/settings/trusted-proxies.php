...
$page_title = $nv_Lang->getModule('trusted_proxy');
...
if ($nv_Request->isset_request('fetch_cloudflare', 'post')) {
    ...
    if (empty($cf_count)) {
        nv_jsonOutput([
            'status' => 'error',
            'mess' => $nv_Lang->getModule('trusted_proxy_cf_error')
        ]);
    }
...
if ($nv_Request->isset_request('save', 'post')) {
    ...
    if (!empty($invalid)) {
        nv_jsonOutput([
            ...
            'mess' => $nv_Lang->getModule('trusted_proxy_invalid', implode(', ', array_slice($invalid, 0, 10)))
        ]);
    }
    ...
    if ($enable and empty($list)) {
        nv_jsonOutput([
            'status' => 'error',
            'input' => 'trusted_proxies',
            'mess' => $nv_Lang->getModule('trusted_proxy_empty_list')
        ]);
    }
