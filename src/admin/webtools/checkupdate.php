            ...
            if (!isset($static_exts[$ext_type]) or !in_array($ext_name, $static_exts[$ext_type], true)) {
                ...
                if (!empty($value['new_version']) and nv_version_compare($value['version'], $value['new_version']) < 0) {
                    ...
                    if (!empty($updateVersion)) {
                        $value['up_link'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=extensions&amp;' . NV_OP_VARIABLE . '=update&amp;eid=' . $value['id'] . '&amp;fid=' . $updateVersion['fid'] . '&amp;checkss=' . csrf_create($admin_info['admin_id'] . '_extensions_update' . '_' . $value['id'] . '_' . $updateVersion['fid']);
                    }
                } elseif (!$value['origin']) {
