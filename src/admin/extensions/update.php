...
if ($nv_Request->get_title('checkss', 'get', '') == md5('unzip' . $eid . $fid . NV_CHECK_SESSION)) {
...
// Tải gói cập nhật
if ($nv_Request->get_title('checkss', 'get', '') == md5('download' . $eid . $fid . NV_CHECK_SESSION)) {
    ...
    $tpl->assign('LINK_UNZIP', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=extensions&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;eid=' . $eid . '&amp;fid=' . $fid . '&amp;checkss=' . md5('unzip' . $eid . $fid . NV_CHECK_SESSION));
...
// Lấy thông tin
if ($nv_Request->get_title('checkss', 'get', '') == md5('check' . $eid . $fid . NV_CHECK_SESSION)) {
    $NV_Http = new NukeViet\Http\Http($global_config, NV_TEMP_DIR);
    $stored_cookies = nv_get_cookies();
    ...
    if ($array['fileInfo'] === 'ready') {
        ...
        $array['link'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=update&eid=' . $eid . '&fid=' . $fid . '&checkss=' . md5('download' . $eid . $fid . NV_CHECK_SESSION);
    } elseif ($array['fileInfo'] == 'notlogin') {
        $redirect = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=update&eid=' . $eid . '&fid=' . $fid . '&checkss=' . md5($eid . $fid . NV_CHECK_SESSION);
...
// Load thông tin chậm
if ($nv_Request->get_title('checkss', 'get', '') == md5($eid . $fid . NV_CHECK_SESSION)) {
    $tpl->assign('EID', $eid);
    $tpl->assign('FID', $fid);
    $tpl->assign('CHECKSS', md5('check' . $eid . $fid . NV_CHECK_SESSION));
