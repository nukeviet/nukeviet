...
foreach ($modules_data as $row) {
    if (array_key_exists($row['basename'], $modules_exit)) {
        ...
        if (array_key_exists($row['title'], $news_modules_for_file)) {
            ...
            $mod['note'] = $row['note'];
            $mod['url_setup'] = array_key_exists($row['title'], $modules_for_title) ? '' : NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;setmodule=' . $row['title'] . '&amp;checkss=' . csrf_create($_csrf_key);
