...
if ($nv_Request->get_title('checkss', 'post') == $checkss) {
    ...
    if (empty($adminpass)) {
        ...
        if ($sendmail) {
            ...
            $title = sprintf($lang_module['delete_sendmail_title'], $gconfigs['site_name']);
            $my_mail = $admin_info['view_mail'] ? $admin_info['email'] : $gconfigs['site_email'];
            if (empty($reason)) {
                $message = sprintf($lang_module['delete_sendmail_mess0'], $gconfigs['site_name'], nv_date('d/m/Y H:i', NV_CURRENTTIME), $my_mail);
            } else {
                $message = sprintf($lang_module['delete_sendmail_mess1'], $gconfigs['site_name'], nv_date('d/m/Y H:i', NV_CURRENTTIME), $reason, $my_mail);
            }
