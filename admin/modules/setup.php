...
foreach( $modules_data as $row )
{
	if( in_array( $row['title'], $modules_exit ) )
	{
		...
		if( array_key_exists( $row['title'], $news_modules_for_file ) )
		{
			...
			$mod['setup'] = "<i class=\"icon-sun icon-large\">&nbsp;</i> <a href=\"" . $url . "\">" . $lang_module['setup'] . "</a>";
			$mod['delete'] = '';
			if( defined( "NV_IS_GODADMIN" ) AND ! in_array( $row['module_file'], $module_virtual_setup ) )
			{
				...
				$mod['delete'] = " - <i class=\"icon-trash icon-large\">&nbsp;</i> <a href=\"" . $url . "\" onclick=\"return confirm(nv_is_del_confirm[0]);\">" . $lang_global['delete'] . "</a>";
			}
