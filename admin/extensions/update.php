...
if ($nv_Request->get_title('checksess', 'get', '') == md5('download' . $eid . $fid . NV_CHECK_SESSION)) {
    ...
    if (!empty($error)) {
        ...
        if ($warning === true) {
            $xtpl->assign('MESSAGE', $nv_Lang->getModule('get_update_warning', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=extensions&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;eid=' . $eid . '&amp;fid=' . $fid . '&amp;checksess=' . md5('unzip' . $eid . $fid . NV_CHECK_SESSION)));

            $xtpl->parse('warning');
            echo $xtpl->text('warning');
        } else {
            $xtpl->assign('MESSAGE', $nv_Lang->getModule('get_update_ok', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=extensions&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;eid=' . $eid . '&amp;fid=' . $fid . '&amp;checksess=' . md5('unzip' . $eid . $fid . NV_CHECK_SESSION)));
...
if ($nv_Request->get_title('checksess', 'get', '') == md5('check' . $eid . $fid . NV_CHECK_SESSION)) {
    ...
    if ($array['fileInfo'] === 'ready') {
        ...
        $xtpl->assign('MESSAGE', $nv_Lang->getModule('extUpdLoginRequire', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=login&amp;redirect=' . nv_redirect_encrypt($redirect)));
        ...
        $xtpl->assign('MESSAGE', $nv_Lang->getModule('extUpdPaidRequire', $array['link'] . '#tabs-1'));
        $xtpl->parse('check.message');
    } else {
        $array['icon'] = 'fa-frown-o';
