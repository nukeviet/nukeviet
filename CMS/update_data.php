...
function nv_up_r1827()
{
	...
	while( list( $lang ) = $db->sql_fetchrow( $language_query ) )
	{
		$mquery = $db->sql_query( "SELECT `title` FROM `" . $db_config['prefix'] . "_" . $lang . "_modules` WHERE `module_file`='weblinks'" );
		while( list( $mod ) = $db->sql_fetchrow( $mquery ) )
		{
			$check = $db->sql_query( "DELETE FROM `" . $db_config['prefix'] . "_" . $lang . "_modfuncs` WHERE `func_name`='link' AND `in_module`='" . $mod . "'" );
			if( ! $check ) break;
		}
	}
