...
if ( $nv_Request->get_int( 'save', 'post' ) )
{
	...
	$arr_menu['description'] = filter_text_input( 'description', 'post', '', 1 );
	...
	{
		$query = "INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data . "_menu` (`id`, `title`,`menu_item`, `description`) VALUES (NULL, " . $db->dbescape( $arr_menu['title'] ) . ",''," . $db->dbescape( $arr_menu['description'] ) . ")";
	...
	{
		$query = "UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "_menu` SET `title`=" . $db->dbescape( $arr_menu['title'] ) . ", `description` =  " . $db->dbescape( $arr_menu['description'] ) . " WHERE `id` =" . $arr_menu['id'] . "";
		$db->sql_query( $query );
		if ( $db->sql_affectedrows() > 0 )
		{
			nv_del_moduleCache( $module_name );
...
if ( ! $all_page )
{
	...
	while ( $row = $db->sql_fetchrow( $query2 ) )
	{
		...
		'menu_item' => $item,//	
		'num' => $b ,//
		//'menu_item' => $global_arr_type[$row['type_menu']], //
		'link_view' => NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;op=add_menu&amp;mid=" . $row['id'],//
...
if ( $nv_Request->isset_request( 'add', 'get' ) )
{
	...
	$type=$nv_Request->get_int( 'type_menu', 'post','' );
