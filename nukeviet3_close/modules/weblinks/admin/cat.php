...
if ( ! empty( $savecat ) )
{
    ...
    $data_content['alias'] = filter_text_input( 'alias', 'post' );
    ...
    else
    {
        if ( $data_content['catid'] == 0 )
        {
            ...
            $query = "INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data . "_cat` (`catid`, `parentid`, `title`, `catimage`, `alias`, `description`, `weight`, `inhome`, `numlinks`, `keywords`, `add_time`, `edit_time`) VALUES (NULL, " . $db->dbescape( $data_content['parentid'] ) . ", " . $db->dbescape( $data_content['title'] ) . ", " . $db->dbescape( $data_content['catimage'] ) . " , " . $db->dbescape( $data_content['alias'] ) . ", " . $db->dbescape( $data_content['description'] ) . ", " . $db->dbescape( $weight ) . ", '1', '3', " . $db->dbescape( $keywords ) . ", UNIX_TIMESTAMP(), UNIX_TIMESTAMP())";
            $idnew = $db->sql_query_insert_id( $query );
            if ( $idnew > 0 )
            {
